...
void CWeaponStatMgun::UpdateCL()
{
	...
	{
		cam_Update(EngineInterface->GetDeltaTime(), g_fov);
...
void CWeaponStatMgun::UpdateBarrelDir()
{
	...
	m_cur_x_rot		= angle_inertion_var(m_cur_x_rot,m_tgt_x_rot,0.5f,3.5f,PI_DIV_6,EngineInterface->GetDeltaTime());
	m_cur_y_rot		= angle_inertion_var(m_cur_y_rot,m_tgt_y_rot,0.5f,3.5f,PI_DIV_6,EngineInterface->GetDeltaTime());
}
...
void CWeaponStatMgun::cam_Update			(float dt, float fov)
{
	...
	Camera()->yaw		= angle_inertion_var(Camera()->yaw,		des_cam_dir.x,	0.5f,	7.5f,	PI_DIV_6,	EngineInterface->GetDeltaTime());
	Camera()->pitch		= angle_inertion_var(Camera()->pitch,	des_cam_dir.y,	0.5f,	7.5f,	PI_DIV_6,	EngineInterface->GetDeltaTime());
