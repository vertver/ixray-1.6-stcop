...
void CStalkerCombatPlanner::finalize			()
{
	...
	object().memory().danger().time_line					(EngineInterface->GetRoundedGlobalTime() + 3000);
...
//	add_condition			(action,eWorldPropertyInSmartCover,		false);
	...
	{
		CStalkerSearchPlanner	*action_ = xr_new<CStalkerSearchPlanner>			(m_object,"search enemy planner");
