...
void CParticlesObject::Init	(LPCSTR p_name, IRender_Sector* S, BOOL bAutoRemove)
{
	...
	dwLastTime				= EngineInterface->GetRoundedGlobalTime();
	mt_dt					= 0;
}
...
void CParticlesObject::Play		(bool bHudMode)
{
	...
	V->Play						();
	dwLastTime					= EngineInterface->GetRoundedGlobalTime()-33ul;
	mt_dt						= 0;
	PerformAllTheWork			(0);
	m_bStopping					= false;
}
...
void CParticlesObject::shedule_Update	(u32 _dt)
{
	...
	// Update
	if (m_bDead)					return;
	u32 dt							= EngineInterface->GetRoundedGlobalTime() - dwLastTime;
	if (dt)							{
		mt_dt					= 0;
		IParticleCustom* V		= smart_cast<IParticleCustom*>(renderable.visual); VERIFY(V);
		V->OnFrame				(dt);
...
{ 
	...
	return EngineInterface->GetCameraState().CameraPosition.distance_to(Position())/200.f; 
}
