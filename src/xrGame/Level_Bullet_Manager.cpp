				   ...
				   bool SendHit)
{
	...
	born_time				= EngineInterface->GetRoundedGlobalTime();
	life_time				= 0.f;
...
//	flags.skipped_frame		= 0;
	...
	init_frame_num			= EngineInterface->GetFrame();
...
//	bullet.frame_num			= EngineInterface->GetFrame();
...
//	VERIFY						( m_thread_id == GetCurrentThreadId() );
	...
	u32 const time_delta		= EngineInterface->GetRoundedDeltaTime();
	if (!time_delta)
		return;
...
//	VERIFY						(data.collide_time <= bullet.life_time + EngineInterface->GetGlobalTime());
...
void CBulletManager::Render	()
{
	...
	for(BulletVecIt it = m_BulletsRendered.begin(); it!=m_BulletsRendered.end(); it++){
		...
		float dist2segSqr	= SqrDistancePointToSegment(EngineInterface->GetCameraState().CameraPosition, bullet->bullet_pos, tracer);
		//---------------------------------------------
		...
		if (EngineInterface->GetCameraState().CameraPosition.distance_to_sqr(bullet->bullet_pos)<(length*length))
		{
			length = EngineInterface->GetCameraState().CameraPosition.distance_to(bullet->bullet_pos) - 0.3f;
		}
