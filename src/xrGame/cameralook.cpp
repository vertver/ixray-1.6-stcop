...
{
	switch (cmd){
	case kCAM_ZOOM_IN:	dist	-= val?val:(rot_speed.z*EngineInterface->GetDeltaTime());	break;
	case kCAM_ZOOM_OUT:	dist	+= val?val:(rot_speed.z*EngineInterface->GetDeltaTime());	break;
	case kDOWN:			pitch	-= val?val:(rot_speed.x*EngineInterface->GetDeltaTime()/factor);	break;
	case kUP:			pitch	+= val?val:(rot_speed.x*EngineInterface->GetDeltaTime()/factor);	break;
	case kLEFT:			yaw		-= val?val:(rot_speed.y*EngineInterface->GetDeltaTime()/factor);	break;
	case kRIGHT:		yaw		+= val?val:(rot_speed.y*EngineInterface->GetDeltaTime()/factor);	break;
	}
...
{
	Fquaternion	new_dir;
	new_dir.slerp		(m_current_dir, m_final_dir, EngineInterface->GetDeltaTime());	//1 sec
