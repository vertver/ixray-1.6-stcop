...
void CMainMenu::Activate	(bool bActivate)
{
	...
	if (	(m_screenshotFrame == EngineInterface->GetFrame())	||
		(m_screenshotFrame == EngineInterface->GetFrame()-1) ||
		(m_screenshotFrame == EngineInterface->GetFrame()+1))	return;
	...
	if(bActivate)
	{
		...
		if (b_is_single) {
			m_Flags.set(flRestorePause, EngineInterface->GetState() == ApplicationState::Paused);
		}
		...
		m_deactivated_frame					= EngineInterface->GetFrame();
...
bool CMainMenu::ReloadUI()
{
	...
	m_activatedScreenRatio		= (float)EngineInterface->GetWidth()/(float)EngineInterface->GetHeight() > (UI_BASE_WIDTH/UI_BASE_HEIGHT+0.01f);
	return true;
}
...
void CMainMenu::OnFrame()
{
	...
	//screenshot stuff
	if(m_Flags.test(flGameSaveScreenshot) && EngineInterface->GetFrame() > m_screenshotFrame  )
	{
	...
	{
		CheckForErrorDlg();
		bool b_is_16_9	= (float)EngineInterface->GetWidth()/(float)EngineInterface->GetHeight() > (UI_BASE_WIDTH/UI_BASE_HEIGHT+0.01f);
...
{
	if(m_startDialog && ((m_deactivated_frame < EngineInterface->GetFrame()+4)||bForce) )
		xr_delete		(m_startDialog);
}
...
{
	if ( m_NeedErrDialog == SessionTerminate && (EngineInterface->GetRoundedGlobalTime() - m_start_time) < 8000 )
		return;
