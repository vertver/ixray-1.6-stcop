...
static bool SimmulateNetworkLag()
{
	...
	if (!max_lag_time || (max_lag_time <= EngineInterface->GetRoundedGlobalTime()))
	{
		CRandom				tmp_random(EngineInterface->GetRoundedGlobalTime());
		max_lag_time		= EngineInterface->GetRoundedGlobalTime() + tmp_random.randI(lag_simmulator_min_ping, lag_simmulator_max_ping);
		return false;
	}
	return true;
}
