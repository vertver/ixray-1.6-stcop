...
void game_sv_Deathmatch::RespawnPlayerAsSpectator(IClient* client)
{
	...
	ps->clear				();
	ps->pItemList.clear		();
	ps->DeathTime			= EngineInterface->GetRoundedGlobalTime() - 1001;
...
void game_sv_Deathmatch::Processing_Victim(game_PlayerState* pVictim, game_PlayerState* pKiller)
{
	...
	pVictim->DeathTime					= EngineInterface->GetRoundedGlobalTime();		
...
void	game_sv_Deathmatch::Update()
{
	...
	case GAME_PHASE_PLAYER_SCORES:
		{
			if ( m_delayedRoundEnd && m_roundEndDelay < EngineInterface->GetRoundedGlobalTime() )
			{
	...
	case GAME_PHASE_INPROGRESS:
		{
			...
			if (pS)
			{
				if (xrSCData->ps->DeathTime + 1000 > EngineInterface->GetRoundedGlobalTime())
				{
//					return;
				}
			}
...
void	game_sv_Deathmatch::OnDelayedRoundEnd( ERoundEnd_Result reason )
{
	...
	m_roundEndDelay = EngineInterface->GetRoundedGlobalTime() + G_DELAYED_ROUND_TIME*1000;
}

void	game_sv_Deathmatch::OnDelayedTeamEliminated()
{
	m_delayedTeamEliminated = true;
	m_TeamEliminatedDelay = EngineInterface->GetRoundedGlobalTime() + G_DELAYED_ROUND_TIME*1000;
}
