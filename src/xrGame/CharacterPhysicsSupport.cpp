...
{
	m_sv_hit = H;
	m_hit_valide_time = EngineInterface->GetRoundedGlobalTime() + hit_valide_time;
	...
	if( m_flags.test( fl_block_hit ) )
	{
		...
		if( EngineInterface->GetRoundedGlobalTime() - m_EntityAlife.GetLevelDeathTime( ) >= 2000 )
...
{
	m_physics_shell_animated_time_destroy = EngineInterface->GetRoundedGlobalTime() + physics_shell_animated_destroy_delay;
...
void CCharacterPhysicsSupport::update_animation_collision		( )
{
	if( animation_collision( ) )
	{
			...
			if( EngineInterface->GetRoundedGlobalTime() > m_physics_shell_animated_time_destroy )
						destroy_animation_collision		( );
	}
}
#ifdef	DEBUG
...
if( dbg_draw_ragdoll_spawn )
{
	...
	//if( false &&  anim_mov_ctrl && anim_mov_blend && anim_mov_blend->blend != CBlend::eFREE_SLOT &&  anim_mov_blend->timeCurrent + EngineInterface->GetDeltaTime()*anim_mov_blend->speed < anim_mov_blend->timeTotal-SAMPLE_SPF-EPS)//.
	//{
	//	const Fmatrix sv_xform = mXFORM;
	//	mXFORM.set( start_xform );
	//	//anim_mov_blend->blendPower = 1;
	//	anim_mov_blend->timeCurrent  += EngineInterface->GetDeltaTime() * anim_mov_blend->speed;
	//	m_pPhysicsShell->AnimToVelocityState( EngineInterface->GetDeltaTime(), 2 * default_l_limit, 10.f * default_w_limit );
	//	mXFORM.set( sv_xform );
	//}
	IKinematics* K=smart_cast<IKinematics*>( m_EntityAlife.Visual( ) );
...
{
	if( m_hit_valide_time < EngineInterface->GetRoundedGlobalTime() || !m_sv_hit.is_valide() )
	{
		if( m_EntityAlife.use_simplified_visual( ) )
			return;
		ActivateShell( NULL );
