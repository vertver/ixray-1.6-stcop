...
//lighting
	if(m_light_render->get_active()){
		...
		{
			int frame;
			u32 clr					= m_lanim->CalculateBGR(EngineInterface->GetGlobalTime(),frame); // тючтЁр•рхЄ т ЇюЁьрЄх BGR
			Fcolor					fclr;
...
void CHelicopter::DieHelicopter()
{
	...
	if(EngineInterface->GetRoundedGlobalTime() != PositionStack.front().dwTime)
		lin_vel.div((EngineInterface->GetRoundedGlobalTime()-PositionStack.front().dwTime)/1000.0f);
