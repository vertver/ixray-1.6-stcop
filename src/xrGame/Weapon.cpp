...
void CWeapon::UpdateCL		()
{
	...
	if( (GetNextState()==GetState()) && IsGameTypeSingle() && H_Parent()==Level().CurrentEntity())
	{
		...
		if(pActor && !pActor->AnyMove() && this==pActor->inventory().ActiveItem())
		{
			if (hud_adj_mode==0 && 
				GetState()==eIdle && 
				(EngineInterface->GetRoundedGlobalTime()-m_dw_curr_substate_time>20000) && 
				!IsZoomed()&&
...
int CWeapon::GetSuitableAmmoTotal( bool use_item_to_spawn ) const
{
	...
	m_BriefInfo_CalcFrame = EngineInterface->GetFrame();
...
//		if(idx==0)					return;
		...
		if(pActor->IsZoomAimingMode())
			m_zoom_params.m_fZoomRotationFactor += EngineInterface->GetDeltaTime()/m_zoom_params.m_fZoomRotateTime;
		else
			m_zoom_params.m_fZoomRotationFactor -= EngineInterface->GetDeltaTime()/m_zoom_params.m_fZoomRotateTime;
...
float CWeapon::GetHudFov() {
	...
	auto zoom = m_HudFovZoom ? m_HudFovZoom : (psHUD_FOV_def * EngineInterface->GetCameraState().FOV / g_fov);
