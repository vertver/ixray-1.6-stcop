	...
	case ATTACK:
		{
			if(actor)
			{
				...
				if(EngineInterface->GetRoundedGlobalTime() - fight_data_from->attack_time < min_attack_delta_time)
					break;

				fight_data_from->attack_time = EngineInterface->GetRoundedGlobalTime();
