...
{
	float one				= 0.1f;
	float tg				= EngineInterface->GetGlobalTime();

	if(!O->object || O->f_time_affected+one > EngineInterface->GetGlobalTime()) 
		return;
...
/*		Msg			(	"Zone[%s]-hit->[%s] Power=%3.3f Frame=%d Time=%3.3f", 
						...
						tg);*/
///		Msg( "Zone hit ___   damage = %.4f    Frame=%d ", send_power, EngineInterface->GetFrame() );
#endif
