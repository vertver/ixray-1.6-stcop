...
void CShootingObject::Light_Start	()
{
	...
	if (EngineInterface->GetFrame()	!= light_frame)
	{
		light_frame					= EngineInterface->GetFrame();
									 ...
									 const Fvector& parent_vel)
{
	...
	if( EngineInterface->GetCameraState().CameraPosition.distance_to_sqr(play_pos)>2*2 ) return;
...
void CShootingObject::UpdateLight()
{
	...
	{
		light_time -= EngineInterface->GetDeltaTime();
								 ...
								 bool send_hit)
{
	...
	if (m_bUseAimBullet)
	{
		if (ParentMayHaveAimBullet())
		{
			...
			{
				if ((EngineInterface->GetGlobalTime()-m_fPredBulletTime)>=m_fTimeToAim)
				{
					aim_bullet=true;
				}
				else
	...
	m_fPredBulletTime = EngineInterface->GetGlobalTime();
