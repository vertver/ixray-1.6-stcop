...
BOOL CPostprocessAnimator::Process(SPPInfo &PPInfo)
{
	...
	if(m_start_time<0.0f)m_start_time=EngineInterface->GetGlobalTime();
	if(m_bCyclic &&((EngineInterface->GetGlobalTime()-m_start_time)>f_length)) m_start_time+=f_length;

	Update					(EngineInterface->GetGlobalTime()-m_start_time);

	VERIFY				(_valid(m_factor));
	VERIFY				(_valid(m_factor_speed));
	VERIFY				(_valid(m_dest_factor));
	if(m_bStop)
		m_factor			-=	EngineInterface->GetDeltaTime()*m_factor_speed;
	else
		m_factor			+= m_factor_speed*EngineInterface->GetDeltaTime()*(m_dest_factor-m_factor);
