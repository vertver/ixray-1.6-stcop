...
{
	fTime			+= EngineInterface->GetDeltaTime();
	if (dwMState&ACTOR_DEFS::mcAnyMove){
		if (fReminderFactor<1.f)	fReminderFactor += SPEED_REMINDER*EngineInterface->GetDeltaTime();
		else						fReminderFactor = 1.f;
	}else{
		if (fReminderFactor>0.f)	fReminderFactor -= SPEED_REMINDER*EngineInterface->GetDeltaTime();
