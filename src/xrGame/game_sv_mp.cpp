...
void	game_sv_mp::Update	()
{
	...
	for (u32 i=0; i<m_CorpseList.size(); )
	{
		...
		if (!pCorpseObj)
		{
			...
			Msg("corpse [%d] not found [%d]",CorpseID, EngineInterface->GetFrame());
			continue;
		}
		if (!pCorpseObj->children.empty())
		{
			Msg("corpse [%d] childern not empty [%d]",CorpseID, EngineInterface->GetFrame());
			i++;
			continue;
		}
		...
		m_CorpseList.erase(m_CorpseList.begin() + i);
		Msg("corpse [%d] send destroy [%d]",CorpseID, EngineInterface->GetFrame());
	}
	...
	{
		int curr_minutes = iFloor(EngineInterface->GetGlobalTime()/60.0f);
...
void	game_sv_mp::SpawnPlayer(ClientID id, LPCSTR N)
{
	...
	if (pA) 
	{
		...
		ps_who->RespawnTime = EngineInterface->GetRoundedGlobalTime();
...
#ifdef MP_LOGGING
	Msg("--- Player [%d] killed player [%d], Frame [%d]", KillerID, KilledID, EngineInterface->GetFrame());
#endif
...
//	R_ASSERT2( entity, "entity not found for destroying" );
	...
	NET_Packet P;
	P.w_begin(M_EVENT);
	P.w_u32(EngineInterface->GetRoundedGlobalTime() - 2*NET_Latency);
	P.w_u16(GE_DESTROY);
	P.w_u16(entity->ID);
	Level().Send(P, net_flags(TRUE,TRUE));
...
{
	if(!bRoundResult)
		ini.w_u32						(sect, "online_time_sec", EngineInterface->GetRoundedGlobalTime()/1000);
}
...
void game_sv_mp::DumpRoundStatisticsAsync()
{
	...
	m_async_stats_request_time = EngineInterface->GetRoundedGlobalTime();
...
bool game_sv_mp::CheckStatisticsReady()
{
	...
	if (m_async_stats.all_ready() || 
		(m_async_stats_request_time + g_sv_dwMaxClientPing) < EngineInterface->GetRoundedGlobalTime())
	{
		DumpRoundStatistics();
