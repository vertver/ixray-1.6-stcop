...
IC	void CSoundMemoryManager::update_sound_threshold			()
{
	...
	m_sound_threshold		= _max(
		...
		m_sound_threshold*
		exp(
			float(EngineInterface->GetRoundedGlobalTime() - m_last_sound_time)/
...
void CSoundMemoryManager::feel_sound_new(CObject *object, int sound_type, CSound_UserDataPtr user_data, const Fvector &position, float sound_power)
{
	...
	VERIFY					(self);
#ifndef SILENCE
	Msg						("%s (%d) - sound type %x from %s at %d in (%.2f,%.2f,%.2f) with power %.2f",*self->cName(),EngineInterface->GetRoundedGlobalTime(),sound_type,object ? *object->cName() : "world",EngineInterface->GetRoundedGlobalTime(),position.x,position.y,position.z,sound_power);
#endif
...
//				if (is_shooting || is_colliding || very_close)
	...
	m_last_sound_time		= EngineInterface->GetRoundedGlobalTime();
	VERIFY					(_valid(m_sound_threshold));
...
#	endif
	...
	if (m_sounds->end() == J) {
		...
		sound_object.m_first_level_time	= EngineInterface->GetRoundedGlobalTime();
#endif
		add						(sound_object);
	}
	else {
...
void CSoundMemoryManager::save	(NET_Packet &packet) const
{
	...
	for ( ; I != E; ++I) {
		...
		packet.w_u32			((EngineInterface->GetRoundedGlobalTime() >= (*I).m_level_time) ? (EngineInterface->GetRoundedGlobalTime() - (*I).m_level_time) : 0);
#endif // USE_LAST_LEVEL_TIME
#ifdef USE_LEVEL_TIME
		packet.w_u32			((EngineInterface->GetRoundedGlobalTime() >= (*I).m_level_time) ? (EngineInterface->GetRoundedGlobalTime() - (*I).m_last_level_time) : 0);
#endif // USE_LAST_LEVEL_TIME
#ifdef USE_FIRST_LEVEL_TIME
		packet.w_u32			((EngineInterface->GetRoundedGlobalTime() >= (*I).m_level_time) ? (EngineInterface->GetRoundedGlobalTime() - (*I).m_first_level_time) : 0);
...
#endif
#ifdef USE_LEVEL_TIME
		VERIFY						(EngineInterface->GetRoundedGlobalTime() >= object.m_level_time);
		object.m_level_time			= packet.r_u32();
		object.m_level_time			+= EngineInterface->GetRoundedGlobalTime();
#endif // USE_LEVEL_TIME
#ifdef USE_LAST_LEVEL_TIME
		VERIFY						(EngineInterface->GetRoundedGlobalTime() >= object.m_last_level_time);
		object.m_last_level_time	= packet.r_u32();
		object.m_last_level_time	+= EngineInterface->GetRoundedGlobalTime();
#endif // USE_LAST_LEVEL_TIME
#ifdef USE_FIRST_LEVEL_TIME
		VERIFY						(EngineInterface->GetRoundedGlobalTime() >= (*I).m_first_level_time);
		object.m_first_level_time	= packet.r_u32();
		object.m_first_level_time	+= EngineInterface->GetRoundedGlobalTime();
#endif // USE_FIRST_LEVEL_TIME
