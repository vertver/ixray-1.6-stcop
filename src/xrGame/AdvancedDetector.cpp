...
void CAdvancedDetector::UpdateAf()
{
	...
	dir_to_artefact.sub		(pCurrentAf->Position(), EngineInterface->GetCameraState().CameraPosition);
	dir_to_artefact.normalize();
	float _ang_af			= dir_to_artefact.getH();
	float _ang_cam			= EngineInterface->GetCameraState().CameraDirection.getH();
	...
	if(af_info.snd_time > af_info.cur_period)
	{
		...
		af_info.snd_time += EngineInterface->GetDeltaTime();
...
/*
	...
	m_curr_ang_speed				= m_curr_ang_speed + a*EngineInterface->GetDeltaTime();
	clamp							(m_curr_ang_speed,-2.0f,2.0f);
	float _add						= m_curr_ang_speed*EngineInterface->GetDeltaTime();
