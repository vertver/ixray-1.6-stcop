...
//	Msg					( "[%6d][%s] weapon_strapped = %s", EngineInterface->GetRoundedGlobalTime(), stalker->cName().c_str(), result ? "true" : "false" );
	return			(result);
}
...
//	Msg					( "[%6d][%s] weapon_unstrapped = %s", EngineInterface->GetRoundedGlobalTime(), stalker->cName().c_str(), result ? "true" : "false" );
	return			(result);
}
