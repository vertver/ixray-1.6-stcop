...
void	imotion_velocity::state_end(  )
{
	VERIFY( shell );
	inherited::state_end( );
	shell->AnimToVelocityState( EngineInterface->GetDeltaTime(), default_l_limit * 10, default_w_limit * 10 );
...
{
		VERIFY( shell );
		if( !shell->AnimToVelocityState( EngineInterface->GetDeltaTime(), 2 * default_l_limit, 10.f * default_w_limit ) )
