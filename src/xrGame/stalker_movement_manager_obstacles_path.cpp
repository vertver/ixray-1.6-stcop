...
bool stalker_movement_manager_obstacles::simulate_path_navigation	()
{
	...
	while (!detail().completed(current_position,!detail().state_patrol_path(),current_travel_point)) {
		...
		if (!m_static_obstacles.process_query(false)) {
			m_last_fail_time		= EngineInterface->GetRoundedGlobalTime();
...
//	Msg								("[%6d] m_failed_to_build_path = %s (stalker_movement_manager_obstacles::build_level_path)",EngineInterface->GetRoundedGlobalTime(),m_failed_to_build_path ? "true" : "false");
...
//	Msg								("[%6d][%6d][%s][%f] build_level_path",EngineInterface->GetFrame(),EngineInterface->GetRoundedGlobalTime(),*object().cName(),timer.GetElapsed_sec()*1000.f);
}
