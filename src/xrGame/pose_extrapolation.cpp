...
{
	if( EngineInterface->GetRoundedGlobalTime() - last_update <  extrapolate_pose_update_mseconds  )
		return;
	m_points.push_back( point().set( pose().set( m ), EngineInterface->GetGlobalTime() ) );
	last_update = EngineInterface->GetRoundedGlobalTime();
}

void	points::init	( const Fmatrix &m )
{
	m_points.fill_in( point().set( pose().set( m ), EngineInterface->GetGlobalTime() ) );
	last_update = EngineInterface->GetRoundedGlobalTime();
}
