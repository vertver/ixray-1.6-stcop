...
{
		s.calc_time			= EngineInterface->GetRoundedGlobalTime();
...
IC bool state_valide( const calculate_state &prev_state )
{
	...
	if( dbg_always_valide )
		return	true;
#endif
	return (EngineInterface->GetRoundedGlobalTime() <= (prev_state.calc_time + UCalc_Interval + EngineInterface->GetRoundedDeltaTime()));
}
