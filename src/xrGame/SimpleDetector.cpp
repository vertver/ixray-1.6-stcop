...
void CUIArtefactDetectorSimple::Flash(bool bOn, float fRelPower)
{
	...
	if(bOn)
	{
		...
		m_turn_off_flash_time = EngineInterface->GetRoundedGlobalTime()+iFloor(fRelPower*1000.0f);
	}else
	{
...
void CUIArtefactDetectorSimple::update()
{
	...
	if(m_parent->HudItemData())
	{
		...
		if(m_turn_off_flash_time && m_turn_off_flash_time<EngineInterface->GetRoundedGlobalTime())
			Flash (false, 0.0f);
		...
		int frame = 0;
		u32 clr					= m_pOnOfLAnim->CalculateRGB(EngineInterface->GetGlobalTime(),frame);
		Fcolor					fclr;
		fclr.set				(clr);
