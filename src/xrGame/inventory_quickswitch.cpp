...
{
	if (m_next_item_iteration_time + exception_items_clear_time <= EngineInterface->GetRoundedGlobalTime())
	{
	...
	if (best_fit)
	{
		...
		m_next_item_iteration_time = EngineInterface->GetRoundedGlobalTime();
		return best_fit;
	}
