...
CLevel::CLevel():IPureClient	(&TheEngine.TimerGlobal)
...
//					GO->H_Parent()->ID(), GO->H_Parent()->cName().c_str(), EngineInterface->GetFrame() );
//			}
		}
		GO->OnEvent		(P,type);
	}
...
void CLevel::ProcessGameEvents		()
{
	// Game events
	{
		...
		if (!game_events->queue.empty())	
			Msg("- d[%d],ts[%d] -- E[svT=%d],[evT=%d]",EngineInterface->GetRoundedGlobalTime(),timeServer(),svT,game_events->queue.begin()->timestamp);
		*/
...
void CLevel::OnFrame	()
{
	...
	if(!g_dedicated_server )
	{
		...
		if( IsGameTypeSingle() && EngineInterface->GetPreCacheFrame()==0 )
		{
			//if (g_mt_config.test(mtMap)) 
	...
	if (bDebug)	{
		for (u32 I=0; I < Level().Objects.o_count(); I++) {
			...
			if (dbg_net_Draw_Flags.test(dbg_draw_skeleton)) //draw skeleton
			{
				...
				{
					if (pGO->Position().distance_to_sqr(EngineInterface->GetCameraState().CameraPosition) < 400.0f)
					{
