...
xrClientData::xrClientData	() : IClient(&TheEngine.TimerGlobal)
{
	ps = NULL;
	Clear		();
}
...
xrServer::xrServer() : IPureServer(&TheEngine.TimerGlobal, g_dedicated_server)
{
...
void		xrServer::client_Destroy	(IClient* C)
{
	...
	if (alife_client)
	{
		...
		if (pS)
		{
			NET_Packet			P;
			P.w_begin			(M_EVENT);
			P.w_u32				(Level().timeServer());//EngineInterface->GetRoundedGlobalTime());
			P.w_u16				(GE_DESTROY);
			P.w_u16				(pS->ID);
...
void xrServer::Update	()
{
	...
	// spawn queue
	u32 svT								= EngineInterface->GetRoundedGlobalTime();
	...
	if( 0==(EngineInterface->GetFrame()%100) )//once per 100 frames
	{
		UpdateBannedList();
	}
}
...
void xrServer::SendUpdatesToAll()
{
	...
	if ((EngineInterface->GetRoundedGlobalTime() - m_last_update_time) >= u32(1000/psNET_ServerUpdate))
	{
		MakeUpdatePackets				();
	...
	case M_REMOTE_CONTROL_AUTH:
		{
			...
			}else
			{
				...
				if(res){
					...
					CL->m_admin_rights.m_dwLoginTime		= EngineInterface->GetRoundedGlobalTime();
					if (CL->ps)
					{
...
if( dbg_net_Draw_Flags.test( dbg_destroy ) )
	...
	R_ASSERT					(P);
	entities.erase				(P->ID);
	m_tID_Generator.vfFreeID	(P->ID,EngineInterface->GetRoundedGlobalTime());
	...
	struct MaxPingClientDisconnector
	{
		...
		void operator()(IClient* client)
		{
			...
			if(	ps->ping > g_sv_dwMaxClientPing && 
				Client->m_ping_warn.m_dwLastMaxPingWarningTime+g_sv_time_for_ping_check < EngineInterface->GetRoundedGlobalTime() )
			{
				++Client->m_ping_warn.m_maxPingWarnings;
				Client->m_ping_warn.m_dwLastMaxPingWarningTime	= EngineInterface->GetRoundedGlobalTime();
...
void xrServer::GetServerInfo( CServerInfo* si )
{
	...
	LPCSTR time = InventoryUtilities::GetTimeAsString( EngineInterface->GetRoundedGlobalTime(), InventoryUtilities::etpTimeToSecondsAndDay ).c_str();
