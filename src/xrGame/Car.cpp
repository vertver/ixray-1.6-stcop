...
void CCar::UpdateEx			(float fov) 
{
	...
	//	Log("UpdateCL",EngineInterface->GetFrame());
	//XFORM().set(m_pPhysicsShell->mXFORM);
	VisualUpdate(fov);
	if(OwnerActor() && OwnerActor()->IsMyCamera()) 
	{
		cam_Update(EngineInterface->GetDeltaTime(), fov);
...
{
	b_starting=true;
	m_dwStartTime=EngineInterface->GetRoundedGlobalTime();
}
...
void CCar::StartBreaking()
{
	...
	{
		b_breaks=true;
		m_break_start=EngineInterface->GetGlobalTime();
	}
}
...
void CCar::UpdateBack()
{
	...
	{
		float k=1.f;
		float time=(EngineInterface->GetGlobalTime()-m_break_start);
...
float CCar::EnginePower()
{
	...
	if(b_starting)
	{
		...
		else if(EngineInterface->GetRoundedGlobalTime()-m_dwStartTime>1000) b_starting=false;
	}
