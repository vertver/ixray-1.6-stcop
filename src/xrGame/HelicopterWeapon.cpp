...
void CHelicopter::MGunUpdateFire()
{
	...
	fShotTimeCounter -= EngineInterface->GetDeltaTime();
	if (delta_t < 0){
		delta_t = EngineInterface->GetGlobalTime();
		flag_by_fire = 0;
	}
	float time_f = EngineInterface->GetGlobalTime() - delta_t;
	...
	if(no_fire_time > 0 && fire_time > 0) {
		if (flag_by_fire==1 && time_f > fire_time){
			delta_t = EngineInterface->GetGlobalTime();
			time_f = EngineInterface->GetGlobalTime() - delta_t;
			flag_by_fire = 0;
		}
...
void CHelicopter::OnShot		()
{
	...
	if(m_enemy.bUseFireTrail){
		...
		float	dt		= EngineInterface->GetGlobalTime() - m_enemy.fStartFireTime; VERIFY(dt>=0);
...
void CHelicopter::MGunFireStart()
{
	...
	if(FALSE==IsWorking() && m_enemy.bUseFireTrail){
		...
		m_enemy.fStartFireTime			= EngineInterface->GetGlobalTime();
...
void CHelicopter::UpdateWeapons		()
{
	...
	// lerp angle
	angle_lerp	(m_cur_rot.x, m_tgt_rot.x, PI, EngineInterface->GetDeltaTime());
	angle_lerp	(m_cur_rot.y, m_tgt_rot.y, PI, EngineInterface->GetDeltaTime());
	...
	if( isOnAttack() ){
		...
		if(m_allow_fire){
			...
			if( between(d,m_min_rocket_dist,m_max_rocket_dist) &&
				(EngineInterface->GetRoundedGlobalTime()-m_last_rocket_attack > m_time_between_rocket_attack) ) {
				...
				m_last_rocket_attack = EngineInterface->GetRoundedGlobalTime();
			}
