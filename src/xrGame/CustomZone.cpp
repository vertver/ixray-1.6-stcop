...
BOOL CCustomZone::net_Spawn(CSE_Abstract* DC) 
{
	...
	if(m_owner_id != u32(-1))
		m_ttl					= EngineInterface->GetRoundedGlobalTime() + 40000;// 40 sec
	else
		m_ttl					= u32(-1);
	...
	m_StartTime					= EngineInterface->GetRoundedGlobalTime();
	...
	m_dwLastTimeMoved			= EngineInterface->GetRoundedGlobalTime();
...
void CCustomZone::UpdateWorkload	(u32 dt)
{
	...
	{
		Fvector P			= EngineInterface->GetCameraState().CameraPosition;
		P.y					-= 0.9f;
		float radius		= 1.0f;
...
void CCustomZone::shedule_Update(u32 dt)
{
	...
	if (IsEnabled())
	{
		...
		float	cam_distance	= EngineInterface->GetCameraState().CameraPosition.distance_to(P)-s.R;
	...
	{
		if(EngineInterface->GetRoundedGlobalTime() > m_ttl)
			DestroyObject ();
	}
}
...
void CCustomZone::UpdateIdleLight	()
{
	...
	int frame = 0;
	u32 clr					= m_pIdleLAnim->CalculateBGR(EngineInterface->GetGlobalTime(),frame); // возвращает в формате BGR
	Fcolor					fclr;
...
void CCustomZone::StartBlowoutLight		()
{
	...
	m_fLightTimeLeft = (float)EngineInterface->GetRoundedGlobalTime() + m_fLightTime*1000.0f;
...
{
	if(m_fLightTimeLeft > (float)EngineInterface->GetRoundedGlobalTime())
	{
		float time_k	= m_fLightTimeLeft - (float)EngineInterface->GetRoundedGlobalTime();

//		m_fLightTimeLeft -= EngineInterface->GetDeltaTime();
...
{
	if(m_dwAffectFrameNum == EngineInterface->GetFrame())	
		return;

	m_dwAffectFrameNum	= EngineInterface->GetFrame();
...
void  CCustomZone::OnMove()
{
	...
	{
		float time_delta	= float(EngineInterface->GetRoundedGlobalTime() - m_dwLastTimeMoved)/1000.f;
		m_dwLastTimeMoved	= EngineInterface->GetRoundedGlobalTime();
...
void CCustomZone::UpdateOnOffState()
{
	...
	bool dest_state;
	u32 t = (EngineInterface->GetRoundedGlobalTime()-m_StartTime+m_TimeShift) % (m_TimeToEnable+m_TimeToDisable);
	if	(t < m_TimeToEnable)
	{
		dest_state=true;
	}else
