...
void CMapLocation::LoadSpot(LPCSTR type, bool bReload)
{
	...
	if ( m_ttl > 0 )
	{
		...
		m_actual_time = EngineInterface->GetRoundedGlobalTime()+m_ttl*1000;
	}
...
const Fvector2& CMapLocation::CalcDirection()
{
	...
	{
		m_cached.m_Direction.set(EngineInterface->GetCameraState().CameraDirection.x,EngineInterface->GetCameraState().CameraDirection.z);
	}else
	{
		CObject* pObject =  Level().Objects.net_Find(m_objectID);
		if(!pObject)
	...
	if(m_flags.test(ePosToActor)){
		...
		if(pObject){
			...
			dcp.set(EngineInterface->GetCameraState().CameraPosition.x, EngineInterface->GetCameraState().CameraPosition.z);
...
{
	R_ASSERT(m_cached.m_updatedFrame!=EngineInterface->GetFrame());
		

	if(	m_flags.test(eTTL) )
	{
		if( m_actual_time < EngineInterface->GetRoundedGlobalTime())
		{
			m_cached.m_Actuality		= false;
			m_cached.m_updatedFrame		= EngineInterface->GetFrame();
