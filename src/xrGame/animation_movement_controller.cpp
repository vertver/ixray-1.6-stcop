...
void	animation_movement_controller::GetInitalPositionBlenSpeed	( )
{
	...
	m_control_blend->timeCurrent+=EngineInterface->GetDeltaTime();
	...
	blend_linear_speed =  l/EngineInterface->GetDeltaTime();
	blend_angular_speed = a/EngineInterface->GetDeltaTime();
...
/*
	Fmatrix res = to;
	blend_linear_speed  += blend_linear_accel *EngineInterface->GetDeltaTime() ;
	blend_angular_speed += blend_angular_accel *EngineInterface->GetDeltaTime() ;
	
	inital_position_blending = !clamp_change( res, m_pObjXForm, blend_linear_speed*EngineInterface->GetDeltaTime(), blend_angular_speed*EngineInterface->GetDeltaTime(), 0.00001, 0.000001 ); 
	m_pObjXForm.set( res );
*/
...
//
//	float change_pos_sq_delta = change_pos_delta * change_pos_delta * (( EngineInterface->GetDeltaTime()/0.01f )*( EngineInterface->GetDeltaTime()/0.01f ));
//
