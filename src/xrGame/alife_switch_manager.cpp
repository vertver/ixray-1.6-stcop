...
#ifdef DEBUG
//	if (psAI_Flags.test(aiALife))
		Msg						("[LSS][%d] Going online [%d][%s][%d] ([%f][%f][%f] : [%f][%f][%f]), on '%s'",EngineInterface->GetFrame(),EngineInterface->GetRoundedGlobalTime(),object->name_replace(), object->ID,VPUSH(graph().actor()->o_Position),VPUSH(object->o_Position), "*SERVER*");
#endif
...
#ifdef DEBUG
//	if (psAI_Flags.test(aiALife))
		Msg							("[LSS][%d] Going offline [%d][%s][%d] ([%f][%f][%f] : [%f][%f][%f]), on '%s'",EngineInterface->GetFrame(),EngineInterface->GetRoundedGlobalTime(),object->name_replace(), object->ID,VPUSH(graph().actor()->o_Position),VPUSH(object->o_Position), "*SERVER*");
#endif
...
void CALifeSwitchManager::try_switch_online	(CSE_ALifeDynamicObject	*I)
{
	...
	VERIFY2						(
		...
		make_string("frame [%d] time [%d] object [%s] with id [%d] is offline, but is on the level",EngineInterface->GetFrame(),EngineInterface->GetRoundedGlobalTime(),I->name_replace(),I->ID)
	);
