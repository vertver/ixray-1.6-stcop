...
void ui_core::pp_start()
{
	...
	m_pp_scale_.set	( float(::Render->getTarget()->get_target_width())/float(UI_BASE_WIDTH),	float(::Render->getTarget()->get_target_height())/float(UI_BASE_HEIGHT) );
	m_2DFrustumPP.CreateFromRect(Frect().set(	0.0f,
												0.0f,
												float(::Render->getTarget()->get_target_width()),
												float(::Render->getTarget()->get_target_height())
												));
