...
void CPHSkeleton::Update(u32 dt)
{
	...
	if(b_removing&&
		EngineInterface->GetRoundedGlobalTime()>m_remove_time&&
		//(EngineInterface->GetRoundedGlobalTime()-m_unsplit_time)*phTimefactor>remove_time&&
...
{
	b_removing=true;
	m_remove_time=EngineInterface->GetRoundedGlobalTime()+iFloor(time/phTimefactor);
	SetNotNeedSave();
