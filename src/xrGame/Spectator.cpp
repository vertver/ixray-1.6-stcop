...
/*			CHudItem* pHudItem = smart_cast<CHudItem*>(pActor->inventory().ActiveItem());
	...
	if (EngineInterface->GetState() == ApplicationState::Paused && pOldActor)
	{
#ifdef DEBUG
...
void CSpectator::cam_Update	(CActor* A)
{
	if (A){
		...
		if (EngineInterface->GetState() == ApplicationState::Paused)
		{
			//EngineInterface->GetDeltaTime() = m_fTimeDelta;	//fake, to update cam (problem with fov)
			g_pGameLevel->Cameras().UpdateFromCamera(cam);
			//EngineInterface->GetDeltaTime() = 0.0f;			//fake, to update cam (problem with fov)
		} else
		{
			g_pGameLevel->Cameras().UpdateFromCamera(cam);
		}
