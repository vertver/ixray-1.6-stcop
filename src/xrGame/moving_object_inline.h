...
{
	m_action_frame		= EngineInterface->GetFrame();
	...
	m_action_time		= EngineInterface->GetRoundedGlobalTime();
#if 0//def DEBUG
	Msg					("%6d %s %s",EngineInterface->GetFrame(),*object().cName(),action == moving_object::action_wait ? "wait" : "move");
#endif // DEBUG
}
