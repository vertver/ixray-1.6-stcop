...
//	CHolderCustom* vehicle=smart_cast<CHolderCustom*>(object);
	...
	Center(center);
	if(m_holder){
		if(!vehicle&& m_holder->Use(EngineInterface->GetCameraState().CameraPosition, EngineInterface->GetCameraState().CameraDirection,center)) detach_Vehicle();
		else{ 
			if(m_holder==vehicle)
				if(m_holder->Use(EngineInterface->GetCameraState().CameraPosition, EngineInterface->GetCameraState().CameraDirection,center))detach_Vehicle();
		}
		return true;
	}else{
		if(vehicle)
		{
			if( vehicle->Use(EngineInterface->GetCameraState().CameraPosition, EngineInterface->GetCameraState().CameraDirection,center))
			{
				if (pCamBobbing)
				{
