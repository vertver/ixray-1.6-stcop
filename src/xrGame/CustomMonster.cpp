...
void CCustomMonster::reinit		()
{
	...
	m_last_client_update_time	= EngineInterface->GetRoundedGlobalTime();
...
void CCustomMonster::shedule_Update	( u32 DT )
{
	...
	m_dwCurrentTime	= EngineInterface->GetRoundedGlobalTime();
...
{ 
	...
	m_client_update_delta		= (u32)std::min(EngineInterface->GetRoundedGlobalTime() - m_last_client_update_time, u32(100) );
	m_last_client_update_time	= EngineInterface->GetRoundedGlobalTime();
...
BOOL CCustomMonster::net_Spawn	(CSE_Abstract* DC)
{
	...
	if (!g_Alive()) {
		set_death_time			();
//		Msg						("%6d : Object [%d][%s][%s] is spawned DEAD",EngineInterface->GetRoundedGlobalTime(),ID(),*cName(),*cNameSect());
	}
...
bool CCustomMonster::update_critical_wounded	(const u16 &bone_id, const float &power)
{
	...
	VERIFY							(EngineInterface->GetRoundedGlobalTime() >= m_last_hit_time);

	if (m_critical_wound_threshold < 0) return (false);


	float							time_delta = m_last_hit_time ? float(EngineInterface->GetRoundedGlobalTime() - m_last_hit_time)/1000.f : 0.f;
