...
{
//	Msg("---------net_Destroy [%d] frame[%d]",ID(), EngineInterface->GetFrame());
...
//	}
	...
	m_time_to_explode			= EngineInterface->GetGlobalTime() + pSettings->r_float(cNameSect(), "force_explode_time")/1000.0f;
#ifdef	DEBUG
	gbg_rocket_speed1=0;
	gbg_rocket_speed2=0;
#endif	
...
//	Msg("! CCustomRocket::OnH_B_Chield called, id[%d] frame[%d]",ID(),EngineInterface->GetFrame());
}
void CCustomRocket::OnH_A_Chield		()
{
	VERIFY(m_eState == eInactive);
	inherited::OnH_A_Chield		();
//	Msg("! CCustomRocket::OnH_A_Chield called, id[%d] frame[%d]",ID(),EngineInterface->GetFrame());
}
...
void CCustomRocket::OnH_A_Independent() 
{
	...
	StartFlying					();
	StartEngine					();
//	Msg("! CCustomRocket::OnH_A_Independent called, id[%d] frame[%d]",ID(),EngineInterface->GetFrame());
...
void CCustomRocket::UpdateCL()
{
	...
	{
		if(m_time_to_explode<EngineInterface->GetGlobalTime())
		{
...
void CCustomRocket::UpdateEnginePh			()
{
	...
	float force = m_fEngineImpulse*fixed_step;// * EngineInterface->GetDeltaTime();
	float k_back=1.f;
	Fvector l_pos, l_dir; 
	l_pos.set(0, 0,-2.f);
	l_dir.set(XFORM().k);
	...
	l_dir.set(0, 1.f, 0);
	force = m_fEngineImpulseUp*fixed_step;// * EngineInterface->GetDeltaTime();
...
void CCustomRocket::UpdateEngine				()
{
	...
	if( !getVisible() ){
		Msg("! CCustomRocket::UpdateEngine called, but false==getVisible() id[%d] frame[%d]",ID(),EngineInterface->GetFrame());
	}
	...
	m_dwEngineTime -= EngineInterface->GetRoundedDeltaTime();
}
