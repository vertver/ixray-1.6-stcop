...
void CProfiler::setup_timer			(LPCSTR timer_id, const u64 &timer_time, const u32 &call_count)
{
	...
	(*i).second.m_update_time	= EngineInterface->GetRoundedGlobalTime();
}
...
void CProfiler::show_stats			(CGameFont *game_font, bool show)
{
	...
	if (!m_portions.empty()) {
		...
		if (!m_actual) {
			...
			TIMERS::iterator	I_ = m_timers.begin();
			TIMERS::iterator	E_ = m_timers.end();
			for ( ; I_ != E_; ++I_)
				max_string_size	= _max(max_string_size,compute_string_length(*(*I_).first));

			I_					= m_timers.begin();
			for ( ; I_ != E_; ++I_)
				convert_string	(*(*I_).first,(*I_).second.m_name,max_string_size);
	...
	for ( ; I != E; ++I) {
		if ((*I).second.m_update_time != EngineInterface->GetRoundedGlobalTime())
			(*I).second.m_time	*= .99f;
