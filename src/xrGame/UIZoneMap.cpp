...
void CUIZoneMap::Update()
{
	...
	if ( !( EngineInterface->GetFrame() % 20 ) && IsGameTypeSingle() )
	{
		string16	text_str;
	...
	UpdateRadar( EngineInterface->GetCameraState().CameraPosition );
	float h, p;
	EngineInterface->GetCameraState().CameraDirection.getHP( h, p );
	SetHeading( -h );
