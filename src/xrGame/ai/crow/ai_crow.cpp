...
{
	if (o_workload_frame	==	EngineInterface->GetFrame())	return;
	o_workload_frame		=	EngineInterface->GetFrame()	;
	switch (st_current)		{
	case eFlyIdle	:
	case eFlyUp		:
		state_Flying		(fdt);
...
{
	UpdateWorkload					(EngineInterface->GetDeltaTime());
	inherited::renderable_Render	();
	o_workload_rframe				= EngineInterface->GetFrame()	;
}
void CAI_Crow::shedule_Update		(u32 DT)
{
	...
	// work
	if (o_workload_rframe	== (EngineInterface->GetFrame()-1))	;
