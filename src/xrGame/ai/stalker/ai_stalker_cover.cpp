...
void CAI_Stalker::on_best_cover_changed				(const CCoverPoint *new_cover, const CCoverPoint *old_cover)
{
	...
	if (new_cover) {
		if (!new_cover->m_is_smart_cover)
			Msg							("[%6d][%s], now it is cover", EngineInterface->GetRoundedGlobalTime(), cName().c_str());
		else
			Msg							(
				"[%6d][%s], now it is smart cover %s",
				EngineInterface->GetRoundedGlobalTime(),
				cName().c_str(),
...
{
#ifdef _DEBUG
//	Msg									("* [%6d][%s] search for new cover performed",EngineInterface->GetRoundedGlobalTime(),*cName());
#endif
#ifdef _DEBUG
...
void CAI_Stalker::update_best_cover_actuality		(const Fvector &position_to_cover_from)
{
	...
	if (m_best_cover->position().distance_to_sqr(position_to_cover_from) < _sqr(MIN_SUITABLE_ENEMY_DISTANCE)) {
		m_best_cover_actual				= false;
#if 0//def _DEBUG
		Msg								("* [%6d][%s] enemy too close",EngineInterface->GetRoundedGlobalTime(),*cName());
#endif
		return;
	}
	...
	if (cover_value >= m_best_cover_value + 1.f) {
		m_best_cover_actual				= false;
#if 0//def _DEBUG
		Msg								("* [%6d][%s] cover became too bad",EngineInterface->GetRoundedGlobalTime(),*cName());
#endif
		return;
	}
...
//		m_best_cover_actual				= false;
//		Msg								("* [%6d][%s] cover became too bad2",EngineInterface->GetRoundedGlobalTime(),*cName());
//		return;
//	}
...
#ifdef _DEBUG
//	Msg									("* [%6d][%s] advance search performed",EngineInterface->GetRoundedGlobalTime(),*cName());
#endif
#ifdef _DEBUG
...
void CAI_Stalker::on_restrictions_change			()
{
	...
	m_best_cover_actual					= false;
#ifdef _DEBUG
	Msg									("* [%6d][%s] on_restrictions_change",EngineInterface->GetRoundedGlobalTime(),*cName());
#endif
}
...
void CAI_Stalker::on_enemy_change					(const CEntityAlive *enemy)
{
	...
	m_best_cover_actual					= false;
#ifdef _DEBUG
//	Msg									("* [%6d][%s] on_enemy_change",EngineInterface->GetRoundedGlobalTime(),*cName());
#endif
}
...
#ifdef _DEBUG
//		Msg								("* [%6d][%s] on_danger_add",EngineInterface->GetRoundedGlobalTime(),*cName());
#endif
		m_best_cover_actual				= false;
	}
}
...
#ifdef _DEBUG
//			Msg							("* [%6d][%s] on_danger_remove",EngineInterface->GetRoundedGlobalTime(),*cName());
#endif
			m_best_cover_actual			= false;
		}
...
{
#ifdef _DEBUG
//	Msg									("* [%6d][%s] cover is blocked",EngineInterface->GetRoundedGlobalTime(),*cName());
#endif
	m_best_cover_actual					= false;
}
