...
bool CAI_Stalker::useful		(const CItemManager *manager, const CGameObject *object) const
{
	...
	if (explosive && smart_cast<const CInventoryItem*>(object))
		agent_manager().location().add	(xr_new<CDangerObjectLocation>(object,EngineInterface->GetRoundedGlobalTime(),DANGER_INFINITE_INTERVAL,DANGER_EXPLOSIVE_DISTANCE));
	...
	if (explosive && (explosive->CurrentParentID() != 0xffff)) {
		...
		if (entity_alive)
			memory().danger().add(CDangerObject(entity_alive,object->Position(),EngineInterface->GetRoundedGlobalTime(),CDangerObject::eDangerTypeGrenade,CDangerObject::eDangerPerceiveTypeVisual,object));
	}
...
void CAI_Stalker::react_on_grenades		()
{
	...
	if (EngineInterface->GetRoundedGlobalTime() < reaction.m_time + GRENADE_INTERVAL)
		return;

//	u32							interval = AFTER_GRENADE_DESTROYED_INTERVAL;
	const CMissile				*missile = smart_cast<const CMissile*>(reaction.m_grenade);
//	if (missile && (missile->destroy_time() > EngineInterface->GetRoundedGlobalTime()))
//		interval				= missile->destroy_time() - EngineInterface->GetRoundedGlobalTime() + AFTER_GRENADE_DESTROYED_INTERVAL;
//	m_object->agent_manager().add_danger_location(reaction.m_game_object->Position(),EngineInterface->GetRoundedGlobalTime(),interval,GRENADE_RADIUS);

	if (missile && agent_manager().member().group_behaviour()) {
//		Msg						("%6d : Stalker %s : grenade reaction",EngineInterface->GetRoundedGlobalTime(),*m_object->cName());
		...
		if (initiator) {
			if (is_relation_enemy(initiator))
				...
				if (missile->Position().distance_to(Position()) < FRIENDLY_GRENADE_ALARM_DIST) {
					u32 const time	= missile->destroy_time() >= EngineInterface->GetRoundedGlobalTime() ? u32(missile->destroy_time() - EngineInterface->GetRoundedGlobalTime()) : 0;
...
void CAI_Stalker::react_on_member_death	()
{
	...
	if (EngineInterface->GetRoundedGlobalTime() < reaction.m_time + TOLLS_INTERVAL)
		return;
