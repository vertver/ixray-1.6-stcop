...
bool CAI_Stalker::bfAssignMovement(CScriptEntityAction *tpEntityAction)
{
	...
	movement().update				(EngineInterface->GetRoundedDeltaTime());
	sight().update					();
...
#ifdef _DEBUG
//		Msg				("%6d Assigning animation : %s",EngineInterface->GetRoundedGlobalTime(),*tpEntityAction->m_tAnimationAction.m_caAnimationToPlay);
#endif
