...
void CPhantom::UpdatePosition(const Fvector& tgt_pos) 
{
	...
	angle_lerp		(vHP.x,tgt_h,fASpeed,EngineInterface->GetDeltaTime());
	angle_lerp		(vHP.y,tgt_p,fASpeed,EngineInterface->GetDeltaTime());

	cur_dir.setHP	(vHP.x,vHP.y);

	Fvector prev_pos=Position();
	XFORM().rotateY (-vHP.x);
	Position().mad	(prev_pos,cur_dir,fSpeed*EngineInterface->GetDeltaTime());
}
