...
void CControlAnimationBase::select_animation(bool anim_end)
{
	...
	m_cur_anim.time_started		= EngineInterface->GetRoundedGlobalTime();
	m_cur_anim.speed._set_current	(1.f);
	m_cur_anim.speed._set_target	(-1.f);
}
...
CMotionDef *CControlAnimationBase::get_motion_def(SAnimItem *it, u32 index)
{
	...
	const MotionID		&motion_id = skeleton_animated->ID_Cycle_Safe(strconcat(sizeof(s2),s2,*it->target_name,_itoa(index,s1,10)));
...
MotionID CControlAnimationBase::get_motion_id(EMotionAnim a, u32 index)
{
	...
	string128			s1,s2;
	return				(smart_cast<IKinematicsAnimated*>(m_object->Visual())->ID_Cycle_Safe(strconcat(sizeof(s2),s2,*anim_it->target_name,_itoa(index,s1,10))));
}
