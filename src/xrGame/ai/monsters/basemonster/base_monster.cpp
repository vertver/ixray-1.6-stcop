...
void CBaseMonster::update_pos_by_grouping_behaviour ()
{
	...
	{
		m_last_grouping_behaviour_update_tick = EngineInterface->GetRoundedGlobalTime();
	}	

	const float dt = 0.001f * (EngineInterface->GetRoundedGlobalTime() - m_last_grouping_behaviour_update_tick);
...
bool CBaseMonster::enemy_accessible ()
{
	...
	if ( EngineInterface->GetRoundedGlobalTime() < m_first_tick_enemy_inaccessible + 3000 )
		return							true;
...
bool CBaseMonster::at_home ()
{
	return										!m_first_tick_object_not_at_home ||
												(EngineInterface->GetRoundedGlobalTime() < m_first_tick_object_not_at_home + 4000);
}
...
void CBaseMonster::update_enemy_accessible_and_at_home_info	()
{
	if ( !Home->at_home() )
	{
		if ( !m_first_tick_object_not_at_home )
			m_first_tick_object_not_at_home	=	EngineInterface->GetRoundedGlobalTime();
	}
	else
		m_first_tick_object_not_at_home		=	0;
	...
	{
		if ( !m_first_tick_enemy_inaccessible )
			m_first_tick_enemy_inaccessible	=	EngineInterface->GetRoundedGlobalTime();

		m_last_tick_enemy_inaccessible		=	EngineInterface->GetRoundedGlobalTime();
	}
	else
	{
		if ( m_last_tick_enemy_inaccessible && EngineInterface->GetRoundedGlobalTime() - m_last_tick_enemy_inaccessible > 3000 )
		{
			m_first_tick_enemy_inaccessible	=	0;
			m_last_tick_enemy_inaccessible	=	0;
		}
...
void CBaseMonster::shedule_Update(u32 dt)
{
	...
	if ( is_paused () )
	{
		dbg_update_cl	= EngineInterface->GetFrame();
		return;
	}
#endif
...
float CBaseMonster::get_screen_space_coverage_diagonal()
{
	...
	Fmatrix				xform;
	xform.mul			(EngineInterface->GetCameraState().FullTransform,XFORM());
