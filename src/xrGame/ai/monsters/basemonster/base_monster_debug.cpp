...
CBaseMonster::SDebugInfo CBaseMonster::show_debug_info()
{
	...
	float y				= 200;
	float x				= (m_show_debug_info == 1) ? 40.f : float(::Render->getTarget()->get_core_width() / 2) + 40.f;
	...
	xr_sprintf(text, "-- Monster : [%s]  Current Time = [%u]", *cName(), EngineInterface->GetRoundedGlobalTime());
...
void   CBaseMonster::add_debug_info (debug::text_tree& root_s)
{
		  ...
		  i!=e; ++i )
	{
		...
		if ( EngineInterface->GetRoundedGlobalTime() >= (*i).m_start_time )
		{
