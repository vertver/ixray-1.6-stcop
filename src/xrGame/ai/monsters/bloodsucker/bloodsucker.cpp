...
void   CAI_Bloodsucker::set_visibility_state (visibility_t new_state)
{
	...
	if ( EngineInterface->GetRoundedGlobalTime() <	m_visibility_state_last_changed_time + 
								get_visibility_state_change_min_delay() )
	{
		return;
	}

	m_visibility_state_last_changed_time	=	EngineInterface->GetRoundedGlobalTime();
...
void   CAI_Bloodsucker::update_invisibility ()
{
	...
	else if ( EngineInterface->GetRoundedGlobalTime() < m_runaway_invisible_time + default_runaway_invisible_time/6 )
	{
		set_visibility_state				(partial_visibility);
	}
	else if ( EngineInterface->GetRoundedGlobalTime() < m_runaway_invisible_time + default_runaway_invisible_time )
	{
