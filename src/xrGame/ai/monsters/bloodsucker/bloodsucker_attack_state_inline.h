...
void   CStateMonsterBackstubEnemy<_Object>::initialize ()
{
	...
	m_encircle_end_tick = EngineInterface->GetRoundedGlobalTime() + detail::bloodsucker::encircle_time;
...
void   CStateMonsterBackstubEnemy<_Object>::execute ()
{
	...
	if ( object->conditions().GetHealth() < m_last_health-detail::bloodsucker::loose_health_diff &&
		 EngineInterface->GetRoundedGlobalTime() > m_next_change_behaviour_tick )
	{
		m_next_change_behaviour_tick = EngineInterface->GetRoundedGlobalTime() + detail::bloodsucker::change_behaviour_time;
		m_last_health = object->conditions().GetHealth();
	...
	if ( EngineInterface->GetRoundedGlobalTime() > m_encircle_end_tick ) 
	{
