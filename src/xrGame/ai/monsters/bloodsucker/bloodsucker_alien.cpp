...
BOOL CAlienEffectorPP::Process(SPPInfo& pp)
{
	...
	def_lerp			(factor,target_factor, PERIOD_SPEED, EngineInterface->GetDeltaTime());
...
BOOL CAlienEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	def_lerp(m_inertion, 1 - rel_dist, rel_dist, EngineInterface->GetDeltaTime());
	...
	def_lerp(m_current_fov, m_target_fov, FOV_SPEED, EngineInterface->GetDeltaTime());
