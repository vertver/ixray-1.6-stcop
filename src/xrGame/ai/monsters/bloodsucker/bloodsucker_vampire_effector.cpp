...
{
	fLifeTime -= EngineInterface->GetDeltaTime(); 
	if(fLifeTime<0) 
		return FALSE;
	...
	if (time_left_perc < 0.2f) {
		...
		angle_lerp(dangle_current.x, dangle_target.x, _abs(dangle_current.x / fLifeTime + 0.001f), EngineInterface->GetDeltaTime());
		angle_lerp(dangle_current.y, dangle_target.y, _abs(dangle_current.y / fLifeTime + 0.001f), EngineInterface->GetDeltaTime());
		angle_lerp(dangle_current.z, dangle_target.z, _abs(dangle_current.z / fLifeTime + 0.001f), EngineInterface->GetDeltaTime());

	} else {
		
		if (angle_lerp(dangle_current.x, dangle_target.x, ANGLE_SPEED, EngineInterface->GetDeltaTime())) {
			dangle_target.x = Random.randFs(DELTA_ANGLE_X);
		}

		if (angle_lerp(dangle_current.y, dangle_target.y, ANGLE_SPEED, EngineInterface->GetDeltaTime())) {
			dangle_target.y = Random.randFs(DELTA_ANGLE_Y);
		}

		if (angle_lerp(dangle_current.z, dangle_target.z, ANGLE_SPEED, EngineInterface->GetDeltaTime())) {
