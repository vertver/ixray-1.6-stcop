...
void CBurer::UpdateGraviObject()
{
	...
	float dt = float(EngineInterface->GetRoundedGlobalTime() - m_gravi_object.time_last_update);
	...
	m_gravi_object.time_last_update		= EngineInterface->GetRoundedGlobalTime();
...
void	CBurer::Hit								(SHit* pHDS)
{
	if ( m_shield_active							&& 
		 ...
		 EngineInterface->GetFrame() != last_hit_frame				) 
	{
		// вычислить позицию и направленность партикла
		Fmatrix pos; 
