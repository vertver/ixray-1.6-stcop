	...
	switch ( m_action )
	{
		case ACTION_TELE_STARTED:
			...
			if ( !time_started )
			{
				...
				time_started					=	EngineInterface->GetRoundedGlobalTime();
			}
			else
			{
...
{
	if (time_started + object->m_tele_time_to_hold > EngineInterface->GetRoundedGlobalTime()) return;
	...
	while (i < object->CTelekinesis::get_objects_count()) {
		...
		if ((tele_object.get_state() == TS_Keep) && (tele_object.time_keep_started + 1500 < EngineInterface->GetRoundedGlobalTime())) {
	...
	if (object_found) {
		...
		if (!IsActiveObjects() || (time_started + MAX_TIME_CHECK_FAILURE < EngineInterface->GetRoundedGlobalTime())) {
			m_action						= ACTION_COMPLETED;
		} 
	}
}
