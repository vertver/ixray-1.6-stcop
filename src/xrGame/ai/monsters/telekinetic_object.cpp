...
bool CTelekineticObject::init(CTelekinesis* tele,CPhysicsShellHolder *obj, float s, float h, u32 ttk, bool rot) 
{
	...
	time_fire_started	= 0;
	//time_raise_started	= EngineInterface->GetRoundedGlobalTime();
...
{
	u32 time=EngineInterface->GetRoundedGlobalTime();
...
{
	//time_keep_started	= EngineInterface->GetRoundedGlobalTime();
	//state				= TS_Keep;
...
{
	if (time_keep_started + time_to_keep < EngineInterface->GetRoundedGlobalTime()) return true;
	return false;
}

bool CTelekineticObject::time_fire_elapsed()
{
	if (time_fire_started + FIRE_TIME < EngineInterface->GetRoundedGlobalTime()) return true;
	return false;
}
...
void CTelekineticObject::keep()
{
	...
	//if (time_keep_updated + KEEP_IMPULSE_UPDATE > EngineInterface->GetRoundedGlobalTime()) return;
	...
	time_keep_updated = EngineInterface->GetRoundedGlobalTime();
...
void CTelekineticObject::fire_t(const Fvector &target, float time)
{
	...
	//time_fire_started	= EngineInterface->GetRoundedGlobalTime();
...
{
	if (time_raise_started + RAISE_MAX_TIME < EngineInterface->GetRoundedGlobalTime())
		return true;
