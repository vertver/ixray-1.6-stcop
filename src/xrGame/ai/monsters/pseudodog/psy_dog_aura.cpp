...
: inherited(ppi)
{
	...
	m_time_state_started	= EngineInterface->GetRoundedGlobalTime();
...
BOOL CPPEffectorPsyDogAura::update()
{
	...
	if (m_effector_state == eStatePermanent) {
		...
		m_factor = float(EngineInterface->GetRoundedGlobalTime() - m_time_state_started) / float(m_time_to_fade);
