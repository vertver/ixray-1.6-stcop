...
{
	float time	=	((EngineInterface->GetRoundedGlobalTime() - m_time_started) / 1000.f) / m_jump_time;
	if ( time > 1.f )
		time	=	1.f;
...
bool CControlJump::can_jump(Fvector const& target, bool const aggressive_jump)
{
	if ( m_time_next_allowed != 0 )
	{
		...
		if ( m_time_next_allowed - (int)aggressive_jump*(2*m_delay_after_jump/3) > EngineInterface->GetRoundedGlobalTime()) 
		{
			return					false;
		}
	}
