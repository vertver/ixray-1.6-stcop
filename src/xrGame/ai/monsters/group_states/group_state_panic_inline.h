...
void CStateGroupPanicAbstract::check_force_state()
{
	if ((current_substate == eStatePanic_FaceUnprotectedArea)){
		// если видит врага
		if (object->EnemyMan.get_enemy_time_last_seen() == EngineInterface->GetRoundedGlobalTime()) {
			select_state(eStatePanic_Run);
			return;
		}
		// если получил hit
		if (object->HitMemory.get_last_hit_time() + 5000 > EngineInterface->GetRoundedGlobalTime()) {
			select_state(eStatePanic_Run);
			return;
		}
	}
