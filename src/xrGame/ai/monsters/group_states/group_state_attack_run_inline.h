...
void CStateGroupAttackRunAbstract::initialize()
{
	...
	// interception
	m_intercept_tick = EngineInterface->GetRoundedGlobalTime();
	...
	// prediction
	m_memorized_tick = EngineInterface->GetRoundedGlobalTime();
	m_memorized_pos  = object->EnemyMan.get_enemy()->Position();
	m_predicted_vel  = cr_fvector3(0.f);

	// encirclement
	if ( EngineInterface->GetRoundedGlobalTime() > m_next_encircle_tick )
	{
		m_encircle_time = 2000 + rand()%4000;
		m_next_encircle_tick = EngineInterface->GetRoundedGlobalTime() + 8000 + rand()%8000;
	}
	else
	{
		m_encircle_time = 0;	
...
{	
 	if ( EngineInterface->GetRoundedGlobalTime() > m_intercept_tick + m_intercept_length )
 	{
 		m_intercept_tick   = EngineInterface->GetRoundedGlobalTime();
 	...
 	if ( EngineInterface->GetRoundedGlobalTime() > m_memorized_tick + memory_update_ms )
 	{
 		m_predicted_vel  = (enemy_pos-m_memorized_pos) * (1000.f / (EngineInterface->GetRoundedGlobalTime()-m_memorized_tick));
	...
	if ( !enemy_at_home || EngineInterface->GetRoundedGlobalTime() < time_state_started + m_encircle_time )
	{
...
// 
// 	//	if ( g_bDogsDirMode )
// 	if ( EngineInterface->GetRoundedGlobalTime()-time_state_started < m_max_encircle_time )
// 	{
//  		CMonsterSquad *squad	= monster_squad().get_squad(object);
//  		if (squad && squad->SquadActive()) {
//  			// Получить команду
