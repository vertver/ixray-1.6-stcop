...
bool CControlMeleeJump::check_start_conditions()
{
	...
	if (m_time_next_melee_jump > EngineInterface->GetRoundedGlobalTime())	return false;
...
void CControlMeleeJump::on_release()
{
	...
	m_time_next_melee_jump = EngineInterface->GetRoundedGlobalTime() + Random.randI(ROTATION_JUMP_DELAY_MIN,ROTATION_JUMP_DELAY_MAX);
}
