...
void CStateMonsterRestFunAbstract::execute()
{
	...
	if ((dist < object->db().m_fDistToCorpse + 0.5f) && (time_last_hit + MIN_DELAY < EngineInterface->GetRoundedGlobalTime())) {
		...
		if  (target && target->m_pPhysicsShell) {
			...
			time_last_hit	= EngineInterface->GetRoundedGlobalTime();
		}
	}
}
...
bool CStateMonsterRestFunAbstract::check_completion()
{
	...
	if (time_state_started + TIME_IN_STATE < EngineInterface->GetRoundedGlobalTime()) return true;
	return false;
}
