...
{	
	if (data.action.time_out !=0) {
		if (time_state_started + data.action.time_out < EngineInterface->GetRoundedGlobalTime()) return true;
	} 
...
{	
	if ( data.action.time_out != 0 )
	{
		if ( time_state_started + data.action.time_out < EngineInterface->GetRoundedGlobalTime() ) 
			return					true;
	} 
	...
	if ( EngineInterface->GetRoundedGlobalTime() < time_state_started + 200 )
	{
