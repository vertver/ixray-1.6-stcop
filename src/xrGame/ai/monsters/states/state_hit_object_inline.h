...
void CStateMonsterHitObjectAbstract::execute()
{
	...
	if (!m_hitted && (time_state_started + TIME_POINTBREAK < EngineInterface->GetRoundedGlobalTime())) {
		m_hitted		= true;
...
{
	if (time_state_started + TIME_OUT_STATE < EngineInterface->GetRoundedGlobalTime()) return true;
	return false;
}
