...
void CMonsterEnemyMemory::update() 
{
	...
	if ( monster->SoundMemory.IsRememberSound() )
	{
		...
		if ( dangerous && EngineInterface->GetRoundedGlobalTime() < sound.time + 2000 )
		{
...
void CMonsterEnemyMemory::add_enemy(const CEntityAlive *enemy)
{
	...
	enemy_info.time		= EngineInterface->GetRoundedGlobalTime();
...
{
	TTime cur_time = EngineInterface->GetRoundedGlobalTime();
