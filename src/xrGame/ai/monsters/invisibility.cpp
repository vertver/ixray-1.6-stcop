...
{
	m_blink				= true;
	m_time_start_blink	= EngineInterface->GetRoundedGlobalTime();
	m_time_last_blink	= 0;
}
...
void CInvisibility::update_blink()
{
	...
	u32 cur_time = EngineInterface->GetRoundedGlobalTime();
...
void CInvisibility::frame_update()
{
	...
	if (!m_manual) {
		if (m_active)	m_energy -= m_speed * EngineInterface->GetDeltaTime();
		else			m_energy += m_speed * EngineInterface->GetDeltaTime();
