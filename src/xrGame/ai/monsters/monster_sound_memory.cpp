...
void CMonsterSoundMemory::UpdateHearing()
{
	...
	Sounds.erase	(
		std::remove_if(
			...
			pred_remove_nonactual_sounds(
				EngineInterface->GetRoundedGlobalTime() - time_memory
			)
		),
		Sounds.end()
	);
	...
	// пересчитать value
	for (xr_vector<SoundElem>::iterator I = Sounds.begin(); I != Sounds.end(); ++I) I->CalcValue(EngineInterface->GetRoundedGlobalTime(), monster->Position());
