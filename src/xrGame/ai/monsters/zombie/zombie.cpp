...
void CZombie::BoneCallback(CBoneInstance *B)
{
	...
	this_class->Bones.Update(B, EngineInterface->GetRoundedGlobalTime());
...
//	inherited::Hit(P,dir,who,element,p_in_object_space,impulse,hit_type);
	...
	if ((pHDS->hit_type == ALife::eHitTypeFireWound) && (EngineInterface->GetFrame() != last_hit_frame)) {
		if (!com_man().ta_is_active() && (time_resurrect + TIME_RESURRECT_RESTORE < EngineInterface->GetRoundedGlobalTime()) && (conditions().GetHealth() < health_death_threshold)) {
			if (conditions().GetHealth() < (health_death_threshold - float(fake_death_count - fake_death_left) * health_death_threshold / fake_death_count)) {
				...
				move().stop					();
				time_dead_start				= EngineInterface->GetRoundedGlobalTime();
...
void CZombie::shedule_Update(u32 dt)
{
	...
	if (time_dead_start != 0) {
		if (time_dead_start + TIME_FAKE_DEATH < EngineInterface->GetRoundedGlobalTime()) {
			time_dead_start  = 0;

			com_man().ta_pointbreak();	

			time_resurrect = EngineInterface->GetRoundedGlobalTime();
		}
	}
}
