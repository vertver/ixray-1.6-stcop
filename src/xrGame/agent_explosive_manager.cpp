...
void CAgentExplosiveManager::register_explosive	(const CExplosive *explosive, const CGameObject *game_object)
{
	...
	m_explosives.push_back				(CDangerExplosive(explosive,game_object,0,EngineInterface->GetRoundedGlobalTime()));
	...
	object().location().add				(xr_new<CDangerObjectLocation>(game_object,EngineInterface->GetRoundedGlobalTime(),interval,GRENADE_RADIUS));
}
