...
void CTorch::UpdateCL() 
{
	...
	if (H_Parent()) 
	{
		...
		if (H_Parent()->XFORM().c.distance_to_sqr(EngineInterface->GetCameraState().CameraPosition)<_sqr(OPTIMIZATION_DISTANCE) || GameID() != eGameIDSingle) {
			// near camera
		...
		if (actor) 
		{
			m_prev_hp.x		= angle_inertion_var(m_prev_hp.x,-actor->cam_FirstEye()->yaw,TORCH_INERTION_SPEED_MIN,TORCH_INERTION_SPEED_MAX,TORCH_INERTION_CLAMP,EngineInterface->GetDeltaTime());
			m_prev_hp.y		= angle_inertion_var(m_prev_hp.y,-actor->cam_FirstEye()->pitch,TORCH_INERTION_SPEED_MIN,TORCH_INERTION_SPEED_MAX,TORCH_INERTION_CLAMP,EngineInterface->GetDeltaTime());
	...
	u32 clr					= lanim->CalculateBGR(EngineInterface->GetGlobalTime(),frame); 
