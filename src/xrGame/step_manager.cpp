...
void CStepManager::on_animation_start(MotionID motion_id, CBlend *blend)
{
	...
	m_time_anim_started = EngineInterface->GetRoundedGlobalTime(); 
...
void CStepManager::update(bool b_hud_view)
{
	...
	float dist_sqr = m_object->Position().distance_to_sqr(EngineInterface->GetCameraState().CameraPosition);
