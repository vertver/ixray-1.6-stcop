...
void CUICursor::InitInternal()
{
	...
	m_b_use_win_cursor	= (screen_size_y >=EngineInterface->GetHeight() && screen_size_x>=EngineInterface->GetWidth());
}
...
void CUICursor::OnRender	()
{
	...
	if( !IsVisible() ) return;
#ifdef DEBUG
	VERIFY(last_render_frame != EngineInterface->GetFrame());
	last_render_frame = EngineInterface->GetFrame();
...
void CUICursor::UpdateCursorPosition(int _dx, int _dy)
{
	...
	if(m_b_use_win_cursor)
	{
		...
		p.x			= (float)pti.x;
		p.y			= (float)pti.y;
		vPos.x		= p.x * (UI_BASE_WIDTH/(float)EngineInterface->GetWidth());
		vPos.y		= p.y * (UI_BASE_HEIGHT/(float)EngineInterface->GetHeight());
	}else
	{
		float sens = 1.0f;
		vPos.x		+= _dx*sens;
...
void CUICursor::SetUICursorPosition(Fvector2 pos)
{
	vPos		= pos;
	POINT		p;
	p.x			= iFloor(vPos.x / (UI_BASE_WIDTH/(float)EngineInterface->GetWidth()));
	p.y			= iFloor(vPos.y / (UI_BASE_HEIGHT/(float)EngineInterface->GetHeight()));
