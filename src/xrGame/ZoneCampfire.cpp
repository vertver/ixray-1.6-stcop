...
void CZoneCampfire::turn_on_script()
{
	...
	{
		m_turn_time				= EngineInterface->GetRoundedGlobalTime()+OVL_TIME;
		m_turned_on				= true;
		GoEnabledState			();
	}
}
...
{
	if(m_turn_time==0 || m_turn_time-EngineInterface->GetRoundedGlobalTime()<(OVL_TIME-2000))
	{
...
{
	if(m_turn_time==0 || m_turn_time-EngineInterface->GetRoundedGlobalTime()<(OVL_TIME-500))
...
void CZoneCampfire::UpdateWorkload(u32 dt)
{
	...
	if(m_turn_time>EngineInterface->GetRoundedGlobalTime())
	{
		float k = float(m_turn_time-EngineInterface->GetRoundedGlobalTime())/float(OVL_TIME);
		...
		if(m_pIdleLight && m_pIdleLight->get_active())
		{
			VERIFY(m_pIdleLAnim);
			int frame = 0;
			u32 clr		= m_pIdleLAnim->CalculateBGR(EngineInterface->GetGlobalTime(),frame);
			Fcolor		fclr;
