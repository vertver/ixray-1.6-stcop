...
void CUILabel::Update()
{
	...
	if (m_lanim)
	{
		if(m_lainm_start_time<0.0f) m_lainm_start_time = EngineInterface->GetGlobalTime(); 
		int frame;
		u32 clr					= m_lanim->CalculateRGB(EngineInterface->GetGlobalTime()-m_lainm_start_time,frame);
		SetColor				(clr);
		SetTextColor			(clr);
	}
}
