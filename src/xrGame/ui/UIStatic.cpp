...
void CUIStatic::Update()
{
	...
	if(m_lanim_xform.m_lanim)
	{
		...
		float t = EngineInterface->GetRoundedGlobalTime()/1000.0f;
	...
	if(CursorOverWindow() && m_stat_hint_text.size() && !g_statHint->Owner() && EngineInterface->GetRoundedGlobalTime()>m_dwFocusReceiveTime+700)
	{
...
{
	m_lanim_xform.m_lanim_start_time = EngineInterface->GetRoundedGlobalTime()/1000.0f;
}
