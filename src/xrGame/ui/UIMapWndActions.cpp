...
void CMapActionZoomControl::init_internal()
{
	...
	m_endMovingTime				= EngineInterface->GetGlobalTime();
...
void CMapActionZoomControl::execute		()
{
	...
	float gt				= EngineInterface->GetGlobalTime();
	float time_to			= m_endMovingTime-gt;
	float dt				= _min(EngineInterface->GetDeltaTime(),time_to);

	if(m_endMovingTime > EngineInterface->GetGlobalTime())
	{
...
void CMapActionMinimize::initialize()
{
	...
	inherited::initialize		();
	m_endMovingTime				= EngineInterface->GetGlobalTime()+map_zoom_time;
}
