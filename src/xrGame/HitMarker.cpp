...
{
	float h1,p1;
	EngineInterface->GetCameraState().CameraDirection.getHP( h1, p1 );
...
{
	m_StartTime						= EngineInterface->GetGlobalTime();
...
{
	return ( ( EngineInterface->GetGlobalTime() - m_StartTime ) < m_lanim->Length_sec() );
}

void SHitMark::Draw( float cam_dir )
{
	int frame;
	u32 clr	= m_lanim->CalculateRGB( EngineInterface->GetGlobalTime() - m_StartTime,frame );
...
SGrenadeMark::SGrenadeMark( const ui_shader& sh, CGrenade* grn )
{
	...
	m_LastTime						= EngineInterface->GetGlobalTime();
...
{
	return ( 2.0f*( EngineInterface->GetGlobalTime() - m_LastTime ) < m_LightAnim->Length_sec() );
}

void SGrenadeMark::Draw( float cam_dir )
{
	int frame;
	u32 clr = m_LightAnim->CalculateRGB( 2.0f*(EngineInterface->GetGlobalTime() - m_LastTime), frame );
	m_UIStaticItem->SetTextureColor( subst_alpha( m_UIStaticItem->GetTextureColor(), color_get_A(clr) ) );
