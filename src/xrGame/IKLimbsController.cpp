...
bool	CIKLimbsController::PredictObjectShift			(  const SCalculateData cd[max_size] )
{
	...
	//{
	//	predict_shift = 0;
	//	predict_time_shift = EngineInterface->GetDeltaTime();
	//}
...
void	CIKLimbsController::ObjectShift	( float static_shift, const SCalculateData cd[max_size])
{
	...
	_object_shift.freeze(EngineInterface->GetState() == ApplicationState::Paused );//ch->interactive_motion() ||
...
void	CIKLimbsController:: Update						( )
{
	...
	/*
	Fmatrix predict;
	_pose_extrapolation.extrapolate( predict, EngineInterface->GetGlobalTime()  ); 
	...
	_pose_extrapolation.extrapolate( predict, EngineInterface->GetGlobalTime() + 1  ); 
	DBG_DrawMatrix( predict, 1 );
	*/
}
