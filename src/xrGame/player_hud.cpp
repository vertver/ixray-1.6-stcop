...
{
	if(!bForce && m_upd_firedeps_frame==EngineInterface->GetFrame())	return;
	...
	m_upd_firedeps_frame			= EngineInterface->GetFrame();
...
void player_hud::render_hud()
{
	...
	::Render->set_PrevTransform	(&m_transform);
	::Render->add_Visual		(m_model->dcast_RenderVisual());
...
void player_hud::update_inertion(Fmatrix& trans)
{
	...
	if (hi)
	{
		...
		// tend to forward
		st_last_dir.mad						(diff_dir, inertion.TendtoSpeed*EngineInterface->GetDeltaTime());
