...
void CLevel::StartPlayDemo()
{
	...
	m_StartGlobalTime	= EngineInterface->GetRoundedGlobalTime();
...
void CLevel::RestartPlayDemo()
{
	...
	m_StartGlobalTime	= EngineInterface->GetRoundedGlobalTime() - m_starting_spawns_dtime;
...
{
	m_demo_header.m_time_global			= EngineInterface->GetRoundedGlobalTime();
...
{
	m_writer->w_u32	(EngineInterface->GetRoundedGlobalTime() - m_demo_header.m_time_global);
...
{
	u32 tdelta = EngineInterface->GetRoundedGlobalTime() - m_StartGlobalTime;
...
{
	return EngineInterface->GetTimeFactor();
}
