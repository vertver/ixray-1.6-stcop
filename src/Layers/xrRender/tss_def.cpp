...
//	TODO: DX10: Implement equivalent for SimulatorStates::record for DX10
	...
	return 0;
#else //USE_DX11
	CHK_DX(RCache.get_Device()->BeginStateBlock());
	for (u32 it=0; it<States.size(); it++)
	{
		State& S	= States[it];
		switch (S.type)
		{
		case 0:	CHK_DX(RCache.get_Device()->SetRenderState		((D3DRENDERSTATETYPE)S.v1,S.v2));				break;
		case 1: CHK_DX(RCache.get_Device()->SetTextureStageState	(S.v1,(D3DTEXTURESTAGESTATETYPE)S.v2,S.v3));	break;
		case 2: 
			{
				CHK_DX(RCache.get_Device()->SetSamplerState		(S.v1,
	...
	CHK_DX	(RCache.get_Device()->EndStateBlock(&SB));
	return	SB;
#endif
}
...
void SimulatorStates::UpdateDesc( D3D_RASTERIZER_DESC &desc ) const
{
	for (u32 it=0; it<States.size(); it++)
	{
		...
		if (S.type==0)
		{
			//CHK_DX(RCache.get_Device()->SetRenderState		((D3DRENDERSTATETYPE)S.v1,S.v2));
			switch (S.v1)
			{
			case D3DRS_FILLMODE:
		...
		//case 1: 
		//	
		//CHK_DX(RCache.get_Device()->SetTextureStageState	(S.v1,(D3DTEXTURESTAGESTATETYPE)S.v2,S.v3));
		//	TODO: DX10: Enable
