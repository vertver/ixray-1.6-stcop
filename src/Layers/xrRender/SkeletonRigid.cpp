...
void CKinematics::CalculateBones			(BOOL bForceExact)
{
	...
	if		(EngineInterface->GetRoundedGlobalTime() == UCalc_Time)										return;	// early out for "fast" update
	UCalc_mtlock	lock	;
	OnCalculateBones		();
	if		(!bForceExact && (EngineInterface->GetRoundedGlobalTime() < (UCalc_Time + UCalc_Interval)))	return;	// early out for "slow" update
	...
	UCalc_Time			= EngineInterface->GetRoundedGlobalTime();
