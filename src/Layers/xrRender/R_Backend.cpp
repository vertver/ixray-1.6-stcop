...
void CBackend::CreateQuadIB		()
{
	...
	//R_CHK(RCache.get_Device()->CreateIndexBuffer(dwIdxCount*2,dwUsage,D3DFMT_INDEX16,D3DPOOL_MANAGED,&QuadIB,NULL));
	...
	//R_CHK(RCache.get_Device()->CreateIndexBuffer(dwIdxCount*2,dwUsage,D3DFMT_INDEX16,D3DPOOL_MANAGED,&QuadIB,NULL));
	R_CHK(RCache.get_Device()->CreateBuffer		( &desc, &subData, &QuadIB));
}
...
void CBackend::CreateQuadIB		()
{
	...
	u32		dwUsage			= D3DUSAGE_WRITEONLY;
	if (HW.Caps.geometry.bSoftware)	dwUsage|=D3DUSAGE_SOFTWAREPROCESSING;
	R_CHK(RCache.get_Device()->CreateIndexBuffer	(dwIdxCount*2,dwUsage,D3DFMT_INDEX16,D3DPOOL_DEFAULT,&QuadIB,NULL));
