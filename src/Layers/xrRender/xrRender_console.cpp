...
public:
	void	apply	()	{
		if (0==RCache.get_Device())	return	;
		int	val = *value;	clamp(val,1,16);
#ifdef USE_DX11
		SSManager.SetMaxAnisotropy(val);
#else //USE_DX11
		for (u32 i=0; i<HW.Caps.raster.dwStages; i++)
			CHK_DX(RCache.get_Device()->SetSamplerState( i, D3DSAMP_MAXANISOTROPY, val	));
#endif //USE_DX11
	}
...
#else //USE_DX11
		for (u32 i = 0; i < HW.Caps.raster.dwStages; i++) {
			CHK_DX(RCache.get_Device()->SetSamplerState(i, D3DSAMP_MIPMAPLODBIAS, val));
		}
#endif
	}
	virtual void Execute(LPCSTR args) {
