...
ICF	float	CalcSSA				(float& distSQ, Fvector& C, dxRender_Visual* V)
{
	...
	distSQ	= EngineInterface->GetCameraState().CameraPosition.distance_to_sqr(C)+EPS;
	return	R/distSQ;
}
...
#if RENDER==R_R1
	if (RI.o.vis_intersect &&	(pVisual->vis.accept_frame!=EngineInterface->GetFrame()))	return;
	pVisual->vis.accept_frame	=	EngineInterface->GetFrame()	;
#endif
