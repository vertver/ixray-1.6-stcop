...
#	include "ResourceManager.h"
	...
	struct ShaderTypeTraits<SHS>
	{
		...
		{
			DXIface* hs = 0;
			R_CHK(RCache.get_Device()->CreateHullShader(buffer, size, NULL, &hs));
			return hs;
		}
	...
	struct ShaderTypeTraits<SDS>
	{
		...
		{
			DXIface* hs = 0;
			R_CHK(RCache.get_Device()->CreateDomainShader(buffer, size, NULL, &hs));
			return hs;
		}
	...
	struct ShaderTypeTraits<SCS>
	{
		...
		{
			DXIface* cs = 0;
			R_CHK(RCache.get_Device()->CreateComputeShader(buffer, size, NULL, &cs));
			return cs;
		}
