...
{
	if (EngineInterface->GetFrame()==L->frame_render)	return;
	L->frame_render							=	EngineInterface->GetFrame();
...
//			Log("sect_name", E.sect_name.c_str());
		...
		OP.mad						(EngineInterface->GetCameraState().CameraPosition,OD,-500.f);
		...
		AD.normalize				();
		AP.mad						(EngineInterface->GetCameraState().CameraPosition,AD,-500.f);
