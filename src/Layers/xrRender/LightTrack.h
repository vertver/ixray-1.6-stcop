...
public:
	...
	virtual float			get_luminocity		()				{ float result_ = _max(approximate.x, _max(approximate.y, approximate.z)); clamp(result_, 0.f, 1.f); return (result_); };
	...
	ICF	float				get_hemi			()	{
		if (dwFrameSmooth!=EngineInterface->GetFrame())		update_smooth();
		return									hemi_smooth;
	}
	ICF	float				get_sun				()	{
		if (dwFrameSmooth!=EngineInterface->GetFrame())		update_smooth();
