...
void Fvisual::Load		(const char* N, IReader *data, u32 dwFlags)
{
	...
	if (!loaded_v && (dwFlags&VLOAD_NOVERTICES)==0) {
		if (data->find_chunk(OGF_VCONTAINER)) {
			...
			BYTE*	bytes		= 0;
			VERIFY				(NULL==p_rm_Vertices);
			R_CHK				(RCache.get_Device()->CreateVertexBuffer	(vCount*vStride,dwUsage,0,D3DPOOL_MANAGED,&p_rm_Vertices,0));
	...
	if (!loaded_v) {
		...
		if (data->find_chunk(OGF_ICONTAINER)) {
			...
			VERIFY				(NULL==p_rm_Indices);
			R_CHK				(RCache.get_Device()->CreateIndexBuffer(iCount*2,dwUsage,D3DFMT_INDEX16,D3DPOOL_MANAGED,&p_rm_Indices,0));
