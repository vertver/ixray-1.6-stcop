...
void CLightProjector::set_object	(IRenderable* O)
{
	...
	{
		if (!O->renderable_ShadowReceive() || RImplementation.val_bInvisible || ((CROS_impl*)O->renderable_ROS())->shadow_recv_frame==EngineInterface->GetFrame())	
		{
			current		= 0;
			return;
		}
		...
		float		D	= C.distance_to	(EngineInterface->GetCameraState().CameraPosition)+R;
		...
		if (current)				{
			...
			LT->shadow_recv_frame	= EngineInterface->GetFrame();
...
void CLightProjector::setup		(int id)
{
	...
	float			dist		= R.C.distance_to	(EngineInterface->GetCameraState().CameraPosition)+Rd;
...
void CLightProjector::calculate	()
{
	...
	for (u32 r_it=0; r_it<receivers.size(); r_it++)
	{
		...
		if (slot<0 || slot>=P_o_count)								bValid = FALSE;	// invalid slot
			...
			if (cache[slot].BB.contains(bb))	{
				...
				if (EngineInterface->GetRoundedGlobalTime() > cache[slot].dwTimeValid)	bValid = FALSE;	// timeout
		...
		if (bValid)			{
			...
			cache[slot].dwFrame	= EngineInterface->GetFrame();
		} else {
	...
	for (u32 c_it=0; c_it<cache.size(); c_it++)
	{
		...
		if (EngineInterface->GetFrame()==cache[c_it].dwFrame)	continue;
		...
		R.dwTimeValid			= EngineInterface->GetRoundedGlobalTime() + ::Random.randI(time_min,time_max);
		...
		if ((v.x*v.x+v.y*v.y+v.z*v.z)<=flt_zero)	{
			...
			R.dwTimeValid			= EngineInterface->GetRoundedGlobalTime();
			LT->shadow_recv_frame	= EngineInterface->GetFrame()-1;
