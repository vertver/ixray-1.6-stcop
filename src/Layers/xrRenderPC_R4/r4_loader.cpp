...
//			D3DVERTEXELEMENT9*	dcl		= (D3DVERTEXELEMENT9*) fs().pointer();
			...
			// Create and fill
			//BYTE*	pData		= 0;
			//R_CHK				(RCache.get_Device()->CreateVertexBuffer(vCount*vSize,dwUsage,0,D3DPOOL_MANAGED,&_VB[i],0));
...
//			fs->advance			(vCount*vSize);
	...
	// Index buffers
	{
		...
		for (u32 i=0; i<count; i++)
		{
			...
			// Create and fill
			//BYTE*	pData		= 0;
			//R_CHK				(RCache.get_Device()->CreateIndexBuffer(iCount*2,dwUsage,D3DFMT_INDEX16,D3DPOOL_MANAGED,&_IB[i],0));
