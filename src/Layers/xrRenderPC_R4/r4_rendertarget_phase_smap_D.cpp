...
void	CRenderTarget::phase_smap_direct		(light* L, u32 sub_phase)
{
	...
	//	R.y1		= L->X.D.minY;
	//	R.y2		= L->X.D.maxY;
	//	CHK_DX							(RCache.get_Device()->Clear( 1L, &R,	  D3DCLEAR_ZBUFFER,	0xFFFFFFFF, 1.0f, 0L));
	//} else {
		// full-clear
	//	CHK_DX							(RCache.get_Device()->Clear( 0L, NULL, D3DCLEAR_ZBUFFER,	0xFFFFFFFF, 1.0f, 0L));
	//}

	RCache.get_Context()->ClearDepthStencilView(rt_smap_depth->pZRT, D3D_CLEAR_DEPTH, 1.0f, 0L);
...
void	CRenderTarget::phase_smap_direct_tsh	(light* L, u32 sub_phase)
{
	...
	//	Prepare viewport for shadow map rendering
	RImplementation.rmNormal();
	RCache.get_Context()->ClearRenderTargetView( RCache.get_RT(0), ColorRGBA);
	//CHK_DX								(RCache.get_Device()->Clear( 0L, NULL, D3DCLEAR_TARGET,	_clr,	1.0f, 0L));
}
