...
void	light::vis_prepare			()
{
	...
	u32	frame	= EngineInterface->GetFrame();
	if (frame	<	vis.frame2test)		return;

	float	safe_area					= VIEWPORT_NEAR;
	{
		float	a0	= deg2rad(EngineInterface->GetCameraState().FOV*EngineInterface->GetCameraState().ASPECT/2.f);
		float	a1	= deg2rad(EngineInterface->GetCameraState().FOV/2.f);
	...
	//Msg	("dist:%f, sa:%f",EngineInterface->GetCameraState().CameraPosition.distance_to(spatial.center),safe_area);
	bool	skiptest	= false;
	...
	if (skiptest || EngineInterface->GetCameraState().CameraPosition.distance_to(spatial.sphere.P)<=(spatial.sphere.R*1.01f+safe_area))	{	// small error
		vis.visible		=	true;
		vis.pending		=	false;
