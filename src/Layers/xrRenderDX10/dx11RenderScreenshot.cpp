...
void CRender::ScreenshotImpl(ScreenshotMode mode, LPCSTR name, CMemoryWriter* memory_writer) {
    ...
    ((ID3D11RenderTargetView*)EngineInterface->GetParent()->GetRenderTarget())->GetResource(&pSrcTexture);
    ...
    case IRender_interface::SM_FOR_GAMESAVE:
    {
        ...
        // Load source texture
        CHK_DX(CaptureTexture(RCache.get_Device(), RCache.get_Context(), pSrcTexture, *pScratchImage));
    ...
    case IRender_interface::SM_FOR_MPSENDING:
    {
        ...
        // Load source texture
        CaptureTexture(RCache.get_Device(), RCache.get_Context(), pSrcTexture, *scratchImage);
    ...
    case IRender_interface::SM_FOR_CUBEMAP:
    {
        ...
        // Create a smaller texture
        CHK_DX(Resize(*pScratchImage->GetImage(0, 0, 0), EngineInterface->GetWidth(), EngineInterface->GetHeight(), TEX_FILTER_FLAGS::TEX_FILTER_LINEAR, *pSmallScratchImage));
...
void CRender::ScreenshotAsyncEnd(CMemoryWriter& memory_writer) {
    ...
    RCache.get_Context()->Map(pTex, 0, D3D_MAP_READ, 0, &MappedData);
    {
    ...
    RCache.get_Context()->Unmap(pTex, 0);
}