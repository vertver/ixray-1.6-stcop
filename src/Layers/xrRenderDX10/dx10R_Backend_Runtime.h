...
IC void CBackend::set_RT(ID3DRenderTargetView* RT, u32 ID)
{
	if (RT!=pRT[ID])
	{
		...
		pRT[ID]			= RT;
		//	Mark RT array dirty
		//RCache.get_Device()->OMSetRenderTargets(sizeof(pRT)/sizeof(pRT[0]), pRT, 0);
		//RCache.get_Device()->OMSetRenderTargets(sizeof(pRT)/sizeof(pRT[0]), pRT, pZB);
		//	Reset all RT's here to allow RT to be bounded as input
		if (!m_bChangedRTorZB)
			RCache.get_Context()->OMSetRenderTargets(0, 0, 0);
...
IC void	CBackend::set_ZB(ID3DDepthStencilView* ZB)
{
	if (ZB!=pZB)
	{
		...
		stat.target_zb	++;
		pZB				= ZB;
		//RCache.get_Device()->OMSetRenderTargets(0, 0, pZB);
		//RCache.get_Device()->OMSetRenderTargets(sizeof(pRT)/sizeof(pRT[0]), pRT, pZB);
		//	Reset all RT's here to allow RT to be bounded as input
		if (!m_bChangedRTorZB)
			RCache.get_Context()->OMSetRenderTargets(0, 0, 0);
...
ICF void CBackend::set_PS(ID3DPixelShader* _ps, LPCSTR _n)
{
	if (ps!=_ps)
	{
		...
		RCache.get_Context()->PSSetShader(ps, 0, 0);
...
ICF void CBackend::set_GS(ID3DGeometryShader* _gs, LPCSTR _n)
{
	if (gs!=_gs)
	{
		...
		RCache.get_Context()->GSSetShader(gs, 0, 0);
...
ICF void CBackend::set_HS(ID3D11HullShader* _hs, LPCSTR _n)
{
	if (hs!=_hs)
	{
		...
		//stat.hs			++;
		hs				= _hs;
		RCache.get_Context()->HSSetShader(hs, 0, 0);
...
ICF void CBackend::set_DS(ID3D11DomainShader* _ds, LPCSTR _n)
{
	if (ds!=_ds)
	{
		...
		//stat.ds			++;
		ds				= _ds;
		RCache.get_Context()->DSSetShader(ds, 0, 0);
...
ICF void CBackend::set_CS(ID3D11ComputeShader* _cs, LPCSTR _n)
{
	if (cs!=_cs)
	{
		...
		//stat.cs			++;
		cs				= _cs;
		RCache.get_Context()->CSSetShader(cs, 0, 0);
...
ICF void CBackend::set_VS(ID3DVertexShader* _vs, LPCSTR _n)
{
	if (vs!=_vs)
	{
		...
		RCache.get_Context()->VSSetShader(vs, 0, 0);
...
ICF void CBackend::set_Vertices(ID3DVertexBuffer* _vb, u32 _vb_stride)
{
	if ((vb!=_vb) || (vb_stride!=_vb_stride))
	{
		...
		//CHK_DX			(RCache.get_Device()->SetStreamSource(0,vb,0,vb_stride));
		//UINT StreamNumber,
		...
		//const UINT *pOffsets
		u32	iOffset = 0;
		RCache.get_Context()->IASetVertexBuffers( 0, 1, &vb, &_vb_stride, &iOffset);
	}
}
...
#endif
		ib				= _ib;
		RCache.get_Context()->IASetIndexBuffer(ib, DXGI_FORMAT_R16_UINT, 0);
	}
}
...
IC void CBackend::ApplyPrimitieTopology( D3D_PRIMITIVE_TOPOLOGY Topology )
{
	if ( m_PrimitiveTopology != Topology )
	{
		...
		RCache.get_Context()->IASetPrimitiveTopology(m_PrimitiveTopology);
	}
}
...
IC void CBackend::Compute(UINT ThreadGroupCountX, UINT ThreadGroupCountY, UINT ThreadGroupCountZ)
{
	...
	constants.flush();
	RCache.get_Context()->Dispatch(ThreadGroupCountX,ThreadGroupCountY,ThreadGroupCountZ);
}

IC void CBackend::Render(D3DPRIMITIVETYPE T_, u32 baseV, u32 startV, u32 countV, u32 startI, u32 PC)
{
	//VERIFY(vs);
	//RCache.get_Device()->VSSetShader(vs);
	//RCache.get_Device()->GSSetShader(0);
	...
	//CHK_DX(RCache.get_Device()->DrawIndexedPrimitive(T,baseV, startV, countV,startI,PC));
...
//	Msg("iIndexCount=%d, startI=%d, baseV=%d", iIndexCount, startI, baseV);
	RCache.get_Context()->DrawIndexed(iIndexCount, startI, baseV);
//	Msg("DrawIndexed: End\n");
...
//	Msg("iVertexCount=%d, startV=%d", iVertexCount, startV);
	//CHK_DX				(RCache.get_Device()->DrawPrimitive(T, startV, PC));
	RCache.get_Context()->Draw(iVertexCount, startV);
//	Msg("Draw: End\n");
...
IC void CBackend::set_Stencil(u32 _enable, u32 _func, u32 _ref, u32 _mask, u32 _writemask, u32 _fail, u32 _pass, u32 _zfail)
{
	...
	// Simple filter
	//if (stencil_enable		!= _enable)		{ stencil_enable=_enable;		CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_STENCILENABLE,		_enable				)); }
	//if (!stencil_enable)					return;
	//if (stencil_func		!= _func)		{ stencil_func=_func;			CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_STENCILFUNC,		_func				)); }
	//if (stencil_ref			!= _ref)		{ stencil_ref=_ref;				CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_STENCILREF,			_ref				)); }
	//if (stencil_mask		!= _mask)		{ stencil_mask=_mask;			CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_STENCILMASK,		_mask				)); }
	//if (stencil_writemask	!= _writemask)	{ stencil_writemask=_writemask;	CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_STENCILWRITEMASK,	_writemask			)); }
	//if (stencil_fail		!= _fail)		{ stencil_fail=_fail;			CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_STENCILFAIL,		_fail				)); }
	//if (stencil_pass		!= _pass)		{ stencil_pass=_pass;			CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_STENCILPASS,		_pass				)); }
	//if (stencil_zfail		!= _zfail)		{ stencil_zfail=_zfail;			CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_STENCILZFAIL,		_zfail				)); }
}
...
IC  void CBackend::set_Z(u32 _enable)
{
	...
	//{ 
	//	z_enable=_enable;
	//	CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_ZENABLE, _enable )); 
	//}
}
...
IC  void CBackend::set_ZFunc(u32 _func)
{
	...
	//{
	//	z_func = _func;
	//	CHK_DX(RCache.get_Device()->SetRenderState( D3DRS_ZFUNC, _func));
	//}
}
...
IC  void CBackend::set_AlphaRef(u32 _value)
{
	...
	//{ 
	//	alpha_ref = _value;
	//	CHK_DX(RCache.get_Device()->SetRenderState(D3DRS_ALPHAREF,_value));
	//}
}
...
IC void	CBackend::set_ColorWriteEnable(u32 _mask )
{
	...
	//	CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_COLORWRITEENABLE,	_mask	));	
	//	CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_COLORWRITEENABLE1,	_mask	));	
	//	CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_COLORWRITEENABLE2,	_mask	));	
	//	CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_COLORWRITEENABLE3,	_mask	));	
	//}
}
ICF void CBackend::set_CullMode(u32 _mode)
{
	StateManager.SetCullMode(_mode);
	//if (cull_mode		!= _mode)		{ cull_mode = _mode;			CHK_DX(RCache.get_Device()->SetRenderState	( D3DRS_CULLMODE,			_mode				)); }
}
...
IC void CBackend::ApplyVertexLayout()
{
	...
	if (it==decl->vs_to_layout.end())
	{
		...
		CHK_DX(RCache.get_Device()->CreateInputLayout(
	...
	if ( m_pInputLayout != it->second)
	{
		...
		RCache.get_Context()->IASetInputLayout(m_pInputLayout);
	}	
}
...
IC void CBackend::set_Constants			(R_constant_table* C_)
{
	...
	//	Setup constant tables
	{
		...
		if (CBuffersNeedUpdate(m_aPixelConstants, aPixelConstants, uiMin, uiMax))
		{
			...
			RCache.get_Context()->PSSetConstantBuffers(uiMin, uiMax-uiMin, &tempBuffer[uiMin]);
		}
		...
		if (CBuffersNeedUpdate(m_aVertexConstants, aVertexConstants, uiMin, uiMax))
		{
			...
			RCache.get_Context()->VSSetConstantBuffers(uiMin, uiMax-uiMin, &tempBuffer[uiMin]);
		}
		...
		if (CBuffersNeedUpdate(m_aGeometryConstants, aGeometryConstants, uiMin, uiMax))
		{
			...
			RCache.get_Context()->GSSetConstantBuffers(uiMin, uiMax-uiMin, &tempBuffer[uiMin]);
		}
		...
		if (CBuffersNeedUpdate(m_aHullConstants, aHullConstants, uiMin, uiMax))
		{
			...
			RCache.get_Context()->HSSetConstantBuffers(uiMin, uiMax-uiMin, &tempBuffer[uiMin]);
		}
		...
		if (CBuffersNeedUpdate(m_aDomainConstants, aDomainConstants, uiMin, uiMax))
		{
			...
			RCache.get_Context()->DSSetConstantBuffers(uiMin, uiMax-uiMin, &tempBuffer[uiMin]);
		}
		...
		if (CBuffersNeedUpdate(m_aComputeConstants, aComputeConstants, uiMin, uiMax))
		{
			...
			RCache.get_Context()->CSSetConstantBuffers(uiMin, uiMax-uiMin, &tempBuffer[uiMin]);
		}
	}
...
ICF void CBackend::ApplyRTandZB()
{
	if (m_bChangedRTorZB)
	{
		...
		get_Context()->OMSetRenderTargets(sizeof(pRT)/sizeof(pRT[0]), pRT, pZB);
	}
}
...
{
	return float(EngineInterface->GetWidth()) * get_scale();
}

inline float CBackend::get_height()
{
	return float(EngineInterface->GetHeight()) * get_scale();
}
