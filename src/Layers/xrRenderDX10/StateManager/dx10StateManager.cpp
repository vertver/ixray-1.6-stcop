...
void dx10StateManager::Apply()
{
	...
	if ( m_bRSNeedApply || m_bRSChanged )
	{
		...
		RCache.get_Context()->RSSetState(m_pRState);
	...
	if ( m_bDSSNeedApply || m_bDSSChanged )
	{
		...
		RCache.get_Context()->OMSetDepthStencilState(m_pDepthStencilState, m_uiStencilRef);
	...
	if ( m_bBSNeedApply || m_bBSChanged )
	{
		...
		RCache.get_Context()->OMSetBlendState(m_pBlendState, BlendFactor, m_uiSampleMask);
