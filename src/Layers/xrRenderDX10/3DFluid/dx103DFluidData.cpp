...
{
	// Create the texture
	CHK_DX( RCache.get_Device()->CreateTexture3D(&TexDesc,NULL,&m_pRTTextures[rtIndex]));
	...
	CHK_DX( RCache.get_Device()->CreateRenderTargetView( m_pRTTextures[rtIndex], &DescRT, &m_pRenderTargetViews[rtIndex]) );

	float color[4] = {0, 0, 0, 0 };

	RCache.get_Context()->ClearRenderTargetView( m_pRenderTargetViews[rtIndex], color );
}
