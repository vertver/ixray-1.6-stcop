...
inline void CObjectList::o_crow		(CObject*	O)
{
	...
	O->dwFrame_AsCrow			= TheEngine.GetFrame();
}
...
void CObject::MakeMeCrow			()
{
	...
	u32 const device_frame_id		= TheEngine.GetFrame();
...
void	CObject::spatial_update		(float eps_P, float eps_R)
{
	...
	if (PositionStack.empty())
	{
		...
		PositionStack.back().dwTime		= TheEngine.GetRoundedGlobalTime();
...
void CObject::UpdateCL			()
{
	...
	if (TheEngine.GetFrame()==dbg_update_cl)								Debug.fatal	(DEBUG_INFO,"'UpdateCL' called twice per frame for %s",*cName());
	dbg_update_cl	= TheEngine.GetFrame();
	...
	{
		float dist = TheEngine.GetCameraState().CameraPosition.distance_to_sqr(Position());
		if (dist < CROW_RADIUS*CROW_RADIUS)	
			MakeMeCrow	();
		else 
		if( (Visual() && Visual()->getVisData().hom_frame+2 > TheEngine.GetFrame()) && (dist < CROW_RADIUS2*CROW_RADIUS2) )
			MakeMeCrow	();
	}
}
...
void CObject::shedule_Update	( u32 T )
{
	...
	else if (EngineInterface->GetCameraState().CameraPosition.distance_to_sqr(Position()) < CROW_RADIUS*CROW_RADIUS)	MakeMeCrow	();
	*/
}
...
void CObject::setDestroy			(BOOL _destroy)
{
	...
	if (_destroy)
	{
		...
		if(debug_destroy)
			Msg("cl setDestroy [%d][%d]",ID(),TheEngine.GetFrame());
#endif
#ifdef MP_LOGGING
		Msg("cl setDestroy [%d][%d]",ID(),TheEngine.GetFrame());
