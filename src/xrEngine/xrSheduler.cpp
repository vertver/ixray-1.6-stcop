...
void CSheduler::internal_Register	(ISheduled* O, BOOL RT)
{
	...
	if (RT)
	{
		// Fill item structure
		Item						TNext;
		TNext.dwTimeForExecute		= TheEngine.GetRoundedGlobalTime();// TheEngine.GetRoundedGlobalTime();
		TNext.dwTimeOfLastExecute = TheEngine.GetRoundedGlobalTime();// TheEngine.GetRoundedGlobalTime();
		TNext.Object				= O;
		...
		// Fill item structure
		Item						TNext;
		TNext.dwTimeForExecute		= TheEngine.GetRoundedGlobalTime();
		TNext.dwTimeOfLastExecute	= TheEngine.GetRoundedGlobalTime();
		TNext.Object				= O;
		TNext.scheduled_name		= O->shedule_Name();
...
{
	// Normal priority
	u32		dwTime					= TheEngine.GetRoundedGlobalTime();
	CTimer							eTimer;
...
//				Msg					("UNREGISTERS unknown object");
		...
		// Real update call
		// Msg						("------- %d:",TheEngine.GetFrame());
#ifdef DEBUG
		T.Object->dbg_startframe = TheEngine.GetFrame();//TheEngine.GetFrame();
		eTimer.Start				();
...
void CSheduler::Update				()
{
...
#ifdef DEBUG_SCHEDULER
	Msg								("SCHEDULER: PROCESS STEP %d",TheEngine.GetFrame());
#endif // DEBUG_SCHEDULER
	// Realtime priority
	...
	for (u32 it=0; it<ItemsRT.size(); it++)
	{
		...
		u32	Elapsed					= dwTime-T.dwTimeOfLastExecute;
#ifdef DEBUG
		VERIFY						(T.Object->dbg_startframe != TheEngine.GetFrame());
		T.Object->dbg_startframe	=TheEngine.GetFrame();
#endif
	...
	m_processing_now				= false;
#ifdef DEBUG_SCHEDULER
	Msg								("SCHEDULER: PROCESS STEP FINISHED %d",TheEngine.GetFrame());
#endif // DEBUG_SCHEDULER
