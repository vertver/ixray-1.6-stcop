...
void setup_lm_screenshot_matrices()
{
    ...
    auto NewCamera = TheEngine.GetCameraState();

    // build camera matrix
    Fbox bb								= curr_lm_fbox;
    bb.getcenter						(NewCamera.CameraPosition);

    NewCamera.CameraDirection.set		( 0.f,-1.f,0.f	);
    NewCamera.CameraTop.set				( 0.f,0.f,1.f	);
    NewCamera.CameraRight.set			( 1.f,0.f,0.f	);
    NewCamera.View.build_camera_dir		(NewCamera.CameraPosition, NewCamera.CameraDirection, NewCamera.CameraTop);

    bb.xform							(NewCamera.View);

    // build project matrix
    NewCamera.Project.build_projection_ortho(bb.max.x-bb.min.x, bb.max.y-bb.min.y, bb.min.z, bb.max.z);
}
...
CDemoRecord::CDemoRecord(const char *name,float life_time) : CEffectorCam(cefDemo,life_time/*,FALSE*/)
{
    ...
    if (file) 
    {
        ...
        m_Camera.invert	(NewCamera.View);
...
BOOL CDemoRecord::ProcessCam(SCamEffectorInfo& info)
{
    ...
    }else
    {
        ...
        m_vT.mul				(m_vVelocity, TheEngine.GetDeltaTime() * speed);
        m_vR.mul				(m_vAngularVelocity, TheEngine.GetDeltaTime() * ang_speed);
        ...
        fLifeTime-= TheEngine.GetDeltaTime();
...
void CDemoRecord::IR_OnKeyboardPress	(int dik)
{
    ...
    if (dik == DIK_PAUSE) {
        auto NewState = TheEngine.GetState() == ApplicationState::Paused ? ApplicationState::Running : ApplicationState::Paused;
        TheEngine.UpdateState(NewState);
    }
}

static void update_whith_timescale( Fvector &v, const Fvector &v_delta )
{
    VERIFY(!fis_zero(TheEngine.GetTimeFactor()));
    float scale = 1.f/ TheEngine.GetTimeFactor();
