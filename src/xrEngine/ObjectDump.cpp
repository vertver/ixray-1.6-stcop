...
ENGINE_API std::string dbg_object_props_dump_string( const CObject *obj )
{
	...
	return make_string( " net_ID :%d, bActiveCounter :%d, bEnabled :%s, bVisible :%s, bDestroy :%s, \n net_Local %s, net_Ready :%s, net_SV_Update :%s, crow :%s, bPreDestroy : %s ", 
		...
		+
		make_string( "\n dbg_update_cl: %d, dwFrame_UpdateCL: %d, dwFrame_AsCrow :%d, TheEngine.GetFrame() :%d, TheEngine.GetRoundedGlobalTime(): %d  \n",
		obj->dbg_update_cl, obj->dwFrame_UpdateCL, obj->dwFrame_AsCrow, TheEngine.GetFrame(), TheEngine.GetRoundedGlobalTime() );
}
