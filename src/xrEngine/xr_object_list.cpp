...
{
	if (TheEngine.GetFrame() == O->dwFrame_UpdateCL) {
#ifdef DEBUG
		//		if (O->getDestroy())
		//			Msg					("- !!!processing_enabled ->destroy_queue.push_back %s[%d] frame [%d]",O->cName().c_str(), O->ID(), TheEngine.GetFrame());
#endif // #ifdef DEBUG
	...
	O->dwFrame_UpdateCL = TheEngine.GetFrame();

	//	Msg							("[%d][0x%08x]IAmNotACrowAnyMore (CObjectList::SingleUpdate)", TheEngine.GetFrame(), dynamic_cast<void*>(O));

	O->UpdateCL();

	VERIFY3(O->dbg_update_cl == TheEngine.GetFrame(), "Broken sequence of calls to 'UpdateCL'", *O->cName());
#if 0//ndef DEBUG
	__try
	{
#endif
...
void CObjectList::clear_crow_vec(Objects& o)
{
	for (u32 _it = 0; _it < o.size(); _it++) {
		//		Msg				("[%d][0x%08x]IAmNotACrowAnyMore (clear_crow_vec)", TheEngine.GetFrame(), dynamic_cast<void*>(o[_it]));
...
{
	if (TheEngine.GetState() != ApplicationState::Paused || bForce)
	{
		// Clients
		if (TheEngine.GetDeltaTime() > EPS_S || bForce)
		{
...
#ifdef DEBUG
			if (debug_destroy)
				Msg("Destroying object[%x][%x] [%d][%s] frame[%d]", dynamic_cast<void*>(O), O, O->ID(), *O->cName(), TheEngine.GetFrame());
#endif // DEBUG
			O->net_Destroy();
			Destroy(O);
		}
...
void		CObjectList::Destroy(CObject* O)
{
	...
	if (TheEngine.GetState() != ApplicationState::Paused) {
		if (!m_crows[1].empty()) {
			...
			VERIFY(TheEngine.GetState() == ApplicationState::Paused || m_crows[1].empty());
			m_crows[1].clear();
		}
	}
	else {
...
void CObjectList::register_object_to_destroy(CObject* object_to_destroy)
{
	...
	for (; it != it_e; ++it)
	{
		...
		{
			Msg("setDestroy called, but not-destroyed child found parent[%d] child[%d]", object_to_destroy->ID(), O->ID(), TheEngine.GetFrame());
			O->setDestroy(TRUE);
		}
	}
