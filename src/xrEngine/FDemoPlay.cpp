...
void CDemoPlay::stat_Start()
{
	...
	stat_started = TRUE;
	Sleep(1);
	stat_StartFrame = TheEngine.GetFrame();
...
void CDemoPlay::stat_Stop()
{
	...
	// total
	u32	dwFramesTotal = TheEngine.GetFrame() - stat_StartFrame;
...
BOOL CDemoPlay::ProcessCam(SCamEffectorInfo& info)
{
	...
	if (m_pMotion)
	{
		...
		m_MParam->Update(TheEngine.GetDeltaTime(), 1.f, true);
	...
	else
	{
		...
		fStartTime += TheEngine.GetDeltaTime();
		...
		for (int i = 0; i < 4; i++) {
			...
			spline1(t, &(v[0]), (Fvector*)&(TheEngine.GetCameraState().View.m[i][0]));
		}
